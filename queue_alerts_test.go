...
func TestQueueAlertWaitBelowHysteresis(t *testing.T) {
	...
	// 等待高于阈值+迟滞，只武装不推送。
	if _, _, fire := queueAlertEvaluateRule(rule, QueueLiveStore{Wait: 100}, 0, state); fire {
	...
	if _, _, fire := queueAlertEvaluateRule(rule, QueueLiveStore{Wait: 50}, 0, state); !fire {
...
func TestQueueAlertCalledReachOnce(t *testing.T) {
	...
	if _, _, fire := queueAlertEvaluateRule(rule, QueueLiveStore{}, 890, state); fire {
		t.Fatal("called 890 vs target-lead 895 should not fire")
	}
	if _, _, fire := queueAlertEvaluateRule(rule, QueueLiveStore{}, 896, state); !fire {
		t.Fatal("called 896 should fire")
	}
	if !state[rule.key()].FiredOnce {
		t.Fatal("called_reach should mark FiredOnce")
	}
	if _, _, fire := queueAlertEvaluateRule(rule, QueueLiveStore{}, 905, state); fire {
