...
全自动抢号 + 排队取号的寿司郎助手。通过本地 MITM 代理捕获微信小程序认证参数，然后自动轮询预约目标时段，并支持到店当天远程取号、定时取号与排队进度跟踪。
...
- **数据看板 / 叫号决策**：在「排队 → 数据看板」输入自己手里的号，即可看到大概几点叫到、建议几点前到店；不输入号码时展示 10:00-22:00 每个时间点一般叫到几号，并给出当前更好去的门店参考。
...
- **全国门店基准采集**：可选周期性快照全部门店等位/状态写入本地 `queue_baseline.jsonl`，作为基准数据；走公开接口，无需认证。
...
- **叫号时间表**：数据看板会把本机记录和线上公开基准转成 10:00-22:00 的叫号表，回答「几点大概叫到多少号」。
