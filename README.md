...
- **排队取号**：可直接远程取号、查号、取消已发出的号；支持「定时取号」——到设定时间自动远程取号（命中后 12 分钟窗口内重试，official 500 自动重试，已存在号则自动恢复并通知），web 进程与后台守护共用按日期独占锁，保证一天只取一次。
...
| `netticket_plan.json` | 定时取号计划（目标门店/时间/状态/号码） |
