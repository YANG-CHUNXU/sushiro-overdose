...
sushiro-overdose sample start    后台静默采样
sushiro-overdose sample autostart on|off   配置系统开机自启动采样
...
- **排队趋势**：用本地排队快照和真实取号记录画折线图，区分工作日、周末和本地节假日表，展示实际过号数与全局过号数。
- **后台采样**：按时间窗和间隔静默刷新目标门店，只记录本地历史数据，不抢号；可配置系统开机自启动，抢号/狙击运行时会自动跳过。
...
| `holidays.json` | 可选节假日/调休工作日本地表，用于排队趋势分类 |
...
排队趋势是纯本地能力，不上传数据。当前只分析本机已捕获、已选择或本地文件里出现过的门店；没有稳定门店 ID 列表时，不能保证一个人自动覆盖全国或某个城市的全部门店。
