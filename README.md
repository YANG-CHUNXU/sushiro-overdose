...
寿司郎到店助手：先帮你看全国门店实时排队、判断手里的号大概几点叫到；需要时再通过本地 MITM 代理捕获微信小程序凭证参数，用于自动抢未来预约、远程取号、定时取号与排队进度跟踪。
...
3. **只看排队 / 叫号预测** → 直接使用公开门店数据，不需要账号凭证
4. **抢预约 / 远程取号 / 读取我的单据** → 再按引导获取凭证参数
5. **设置偏好** → 选择人数、桌型、目标时段、门店顺序
6. **执行前确认** → 抢预约、取号、取消预约和取消排队号都会有明确确认

> **Windows 用手机获取凭证（主路径）：** Windows 上 PC 微信抓不到凭证，需要用手机拿一次。点首页「用手机获取凭证」打开向导，整个过程就三步：
>
...
- **安全分级**：界面明确标注「只读」「需要凭证」「会执行操作」。看排队和叫号预测不会影响账号；抢预约、远程取号和取消类动作会再次确认。
