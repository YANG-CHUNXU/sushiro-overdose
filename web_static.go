...
<main class="shell wrap">
      ...
      <aside class="side">
        ...
        <div class="card">
          ...
          <div class="track">
            ...
            <div class="step" id="step-sampling"><div class="mark">3</div><div><b>信息收集</b><span>先观察关心门店，判断更合适的到店时间。</span></div></div>
            <div class="step" id="step-booking"><div class="mark">4</div><div><b>按优先级抢号</b><span>门店、日期和时段策略会决定尝试顺序。</span></div></div>
          </div>
  ...
  <section id="p-qt" class="hid">
    <div class="cd">
      <div class="fl ai jb mb16 fw g8"><div><div class="cd-t" style="margin-bottom:0">到店预测</div><p class="mu mt8">先收集你关心门店的排队和预约变化，用来判断更适合到店的时间；预测仅供参考。</p></div><div class="fl g8 fw"><button class="bt bt-w bt-s" onclick="loadQueueTrends()">刷新预测</button><button class="bt bt-r bt-s" onclick="setBootSampling(true)">启用开机收集</button></div></div>
  ...
  <section id="p-se" class="hid">
    <div class="settings-grid">
      ...
      <div class="cd" style="grid-column:1/-1">
        <div class="fl ai jb mb16 fw g8"><div class="cd-t" style="margin-bottom:0">本机诊断</div><div class="fl g8 fw"><button class="bt bt-w bt-s" onclick="lD()">刷新</button><button class="bt bt-w bt-s" onclick="copyDiag()">复制诊断</button><button class="bt bt-w bt-s" onclick="repairP()">修复代理</button><button class="bt bt-w bt-s" onclick="stopProcesses()">停止本应用进程</button><button class="bt bt-o bt-s" onclick="uninstallAll()">卸载清理</button></div></div>
...
function uD(){
  ...
  setStep('step-capture',hc?'done':'active');setStep('step-prefs',hc?'active':'');setStep('step-sampling',spState.running?'active':(spState.sample_runs>0?'done':''));setStep('step-booking',hc&&es.status==='booking'?'active':es.status==='success'?'done':'');
  if(es.status==='capturing'){
    ...
    nc.classList.remove('hid');nc.innerHTML=esc(explainMsg(es.message));
  }else if(!hc){
    ...
    nc.classList.remove('hid');nc.textContent='完成认证后，可以先进行信息收集观察门店，也可以直接设置抢号优先级。';
  }else{
    badge.textContent='准备就绪';title.textContent='先看门店，再按优先级抢号';copy.textContent='当前认证已保存。你可以先用信息收集判断到店时间，再按门店、日期和时段优先级抢预约号。';
...
async function lR(){try{const d=await(await fetch('/api/reservations')).json(),c=el('rc');if(d.error){c.innerHTML='<div class="empty">'+esc(d.error)+'</div>';return}if(!d.length){c.innerHTML='<div class="empty">暂无预约</div>';return}c.innerHTML='<div class="sg">'+d.map(r=>{const when=r.slot_label||[r.queueDate,fT(r.start),r.end?'-'+fT(r.end):''].filter(Boolean).join(' '),store=r.store_name||r.monitored_store_id||r.storeId||'';return'<div class="sl av"><div class="tm">'+esc(r.number||'-')+'</div><div class="ss">'+esc(r.status||'-')+(store?' · '+esc(store):'')+'</div><div class="mu mt8">'+esc(when||'时间待确认')+'<br>#'+esc(r.ticketId||'')+'</div></div>'}).join('')+'</div>'}catch(e){el('rc').innerHTML='<div class="empty">加载失败</div>'}}
