...
<main class="shell wrap">
    ...
    <div class="grid">
      <div>
        ...
        <div class="summary">
          ...
          <div class="mini"><span>采样</span><strong id="sumSampling">未启动</strong><p id="sumSamplingSub">用于历史洞察</p></div>
        </div>
      </div>
      <aside class="side">
        ...
        <div class="card">
          ...
          <div class="track">
            ...
            <div class="step" id="step-sampling"><div class="mark">3</div><div><b>积累数据（可选）</b><span>后台采样会让洞察更准。</span></div></div>
            <div class="step" id="step-booking"><div class="mark">4</div><div><b>开始抢号</b><span>后台持续查询，成功后通知。</span></div></div>
          </div>
          <div class="fl g8 fw mt16"><button class="bt bt-w bt-s" onclick="go('se',document.querySelector('[onclick*=se]'))">去设置模板</button><button class="bt bt-w bt-s" onclick="go('sm',document.querySelector('[onclick*=sm]'))">去采样页</button></div>
        </div>
        <div class="card">
          <h2>当前偏好</h2>
  ...
  <section id="p-sm" class="hid">
    <div class="cd">
      ...
      <div class="fg"><label>采样门店</label><div id="samplingStores" class="chips"><span class="mu">加载中</span></div><div id="sampleStoreHint" class="ps mt8"></div></div>
...
function uSamplingSummary(){const s=spState||{};el('sumSampling').textContent=s.running?'运行中':(s.enabled?'已启用':'未启动');el('sumSamplingSub').textContent=s.message||'用于历史洞察'}
