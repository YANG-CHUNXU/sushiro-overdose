  ...
  <section id="p-se" class="hid">
    <div class="settings-grid">
      ...
      <div class="cd" style="grid-column:1/-1">
        <div class="fl ai jb mb16 fw g8"><div class="cd-t" style="margin-bottom:0">本机诊断</div><div class="fl g8 fw"><button class="bt bt-w bt-s" onclick="lD()">刷新</button><button class="bt bt-w bt-s" onclick="repairP()">修复代理</button><button class="bt bt-w bt-s" onclick="stopProcesses()">停止本应用进程</button><button class="bt bt-o bt-s" onclick="uninstallAll()">卸载清理</button></div></div>
...
function diagDetail(d){const cfg=d.config||{},cert=d.certificate||{},pm=d.proxy_marker||{},sp=d.system_proxy||{},net=d.network||{},logs=d.engine_log_tail||[],ports=d.ports||[];const badPorts=ports.filter(p=>!p.available&&!p.current&&!p.fallback_port).map(p=>p.name+': '+(p.error||'占用')),portNotes=ports.filter(p=>p.note).map(p=>p.name+': '+p.note);let html='<b>下一步建议</b><br>';if(!cfg.complete)html+='先重新获取认证参数。<br>';if(cert.cert_exists&&!cert.trusted)html+='证书已生成但未信任，请在系统证书管理中信任，或重新获取认证触发安装。<br>';if(pm.stale||pm.active)html+='发现代理状态，请先点“修复代理”。<br>';if(!net.reachable)html+='寿司郎网络不可达，先确认网络或稍后重试。<br>';html+='<br><b>证书</b>：<code>'+esc(cert.cert_path||'-')+'</code>'+(cert.trust_error?'<br>'+esc(cert.trust_error):'');if(badPorts.length||portNotes.length)html+='<br><b>端口</b>：'+esc(badPorts.concat(portNotes).join('；'));if((sp.summary||[]).length)html+='<br><b>系统代理</b>：'+esc(sp.summary.join('；'));if(logs.length)html+='<br><b>最近日志</b><br>'+logs.slice(-5).map(l=>esc((l.time||'')+' '+(l.message||''))).join('<br>');return html}
async function lD(){const box=el('dg'),detail=el('ddetail');if(!box)return;box.innerHTML='<div class="ci">诊断中</div>';if(detail)detail.classList.add('hid');try{const d=await(await fetch('/api/diagnostics')).json();lastDiag=d;const cfg=d.config||{},cert=d.certificate||{},pm=d.proxy_marker||{},sp=d.system_proxy||{},eng=d.engine||{},net=d.network||{},dp=d.ports||[];const miss=(cfg.missing||[]).join('、'),portIssues=dp.filter(p=>p.in_use&&!p.current&&!p.fallback_port).map(p=>p.name),portNotes=dp.filter(p=>p.note).map(p=>p.note),portText=portIssues.length?portIssues.join('、'):(portNotes.length?portNotes.join('、'):'默认端口可用');const items=[];items.push(chip('认证参数',cfg.complete?'完整':(miss||'未捕获'),cfg.complete?'ok':'bad'));items.push(chip('门店',cfg.store_count?cfg.store_count+' 个':'未选择',cfg.store_count?'ok':'bad'));items.push(chip('证书',cert.trusted?'已信任':cert.cert_exists?'未信任':'未生成',cert.trusted?'ok':'bad'));items.push(chip('端口',portText,portIssues.length?'bad':portNotes.length?'warn':'ok'));items.push(chip('代理残留',pm.stale?'发现残留':pm.active?'运行中':'未发现',pm.stale?'bad':pm.active?'warn':'ok'));items.push(chip('系统代理',sp.available?'可读取':'不可读取',sp.available?'ok':'warn'));items.push(chip('网络',net.reachable?'寿司郎可达':'不可达',net.reachable?'ok':'bad'));items.push(chip('通知',cfg.notification_channels?.length?cfg.notification_channels.join('、'):'未配置',cfg.notification_channels?.length?'ok':'warn'));items.push(chip('引擎',eng.status||'idle',eng.status==='error'?'bad':(eng.status==='booking'||eng.status==='capturing'||eng.status==='sniping')?'warn':'ok'));box.innerHTML=items.join('');if(detail){detail.innerHTML=diagDetail(d);detail.classList.remove('hid')}}catch(e){box.innerHTML='<div class="ci bad">诊断加载失败</div>'}}
