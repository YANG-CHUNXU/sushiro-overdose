...
function renderNetTicketStatus(p){const box=el('ntStatus');if(!box)return;p=p||{};const store=esc(p.store_name||p.store_id||''),tt=p.target_time?netTimeDisp(p.target_time):'';if(!p.enabled){box.innerHTML='<span class="mu">选门店和时间，点「启用定时取号」即可到点自动取号；取到的号可在「我的预约」取消。</span>';return}switch(p.status){case 'success':box.innerHTML='<b>🎫 已自动取号 '+esc(p.number||'(详见我的预约)')+'</b><div class="mu mt8">'+store+' · 去「抢预约 → 我的预约」可取消。</div>';break;case 'error':box.innerHTML='<b>⚠️ 取号失败</b><div class="mu mt8">'+store+' '+tt+'：'+esc(p.last_error||'未知错误')+'<br>改时间后重新启用可重试。</div>';break;case 'expired':box.innerHTML='<b>⏰ 未在窗口内取到号</b><div class="mu mt8">'+store+' '+tt+'：超时已放弃，可重新启用。</div>';break;default:box.innerHTML='<b>⏳ 已设定：'+tt+' 自动取号</b><div class="mu mt8">'+store+' · 到点(约 '+tt+')自动远程取号并推送。保持本应用或后台收集运行即可。</div>'}}
...
async function loadDiscoveryRecords(){const box=el('discRecords');if(!box)return;box.innerHTML='<span class="mu">读取调试记录中...</span>';try{const d=await safeFetch('/api/discovery/records?limit=80');const records=d.records||[];if(!records.length){box.innerHTML='<span class="mu">暂无调试记录。开启调试后重新获取认证，并在小程序里点击“我的预约”等页面。</span>';return}box.innerHTML='<div class="disc-list">'+records.slice().reverse().map(r=>'<div class="sl '+((r.status||0)>=400?'full':'av')+'"><div class="fl ai jb g8 fw"><div class="ss">'+esc(discTime(r.timestamp))+' · '+esc(r.method||'-')+' '+esc(r.path||'-')+'</div><span class="pill">'+esc(r.status||'-')+'</span></div><div class="mu mt8">query：'+esc(discJoin(r.query_keys))+'</div><div class="mu">body keys：'+esc(discJoin(r.request_body_keys))+'</div><div class="mu">response：'+esc(discResp(r))+'</div></div>').join('')+'</div>'}catch(e){box.innerHTML='<div class="ci bad">读取调试记录失败：'+esc(String(e.message||e))+'</div>'}}
