...
.tag,.safe-tag{display:inline-flex;align-items:center;height:24px;padding:0 9px;border-radius:999px;border:1px solid var(--line-strong);background:#fff;color:var(--sub);font-size:11px;font-weight:950;white-space:nowrap}
.tag.read,.safe-read{color:var(--green);border-color:#BFE4CC;background:var(--green-soft)}
.tag.auth,.safe-auth{color:var(--blue);border-color:#B9D2E4;background:var(--blue-soft)}
.tag.action,.safe-action{color:var(--red);border-color:#F0B7B9;background:var(--red-soft)}
.safety-grid{display:grid;gap:9px}
.safety-card{padding:12px;border:1px solid var(--line);border-radius:12px;background:#FBFAF8}
.safety-card b{display:flex;align-items:center;gap:8px;color:var(--ink);font-size:13px}
.safety-card p{margin-top:6px;color:var(--sub);font-size:12px;line-height:1.55}
...
<header class="hdr">
  <div class="shell hdr-in">
    ...
    <nav class="nav top">
      ...
      <a href="#" data-group="book" onclick="goGroup('book');return false">预约未来</a>
      <a href="#" data-group="auto" onclick="goGroup('auto');return false">自动蹲号</a>
...
<main class="shell wrap">
    ...
    <div class="grid">
      <div>
        <div class="hero">
          ...
          <div class="actions">
            ...
            <button class="bt bt-w" id="bc" onclick="startAuth()">获取凭证</button>
            <button class="bt bt-w" id="brst" onclick="rST()">高级：重置抓包</button>
          </div>
          <div id="nc" class="notice hid"></div>
        </div>
        <div class="task-grid">
          <button class="task-card" onclick="go('qt')" type="button">
            <span class="tag read">只读 · 不用凭证</span>
            <h3>现在想去吃</h3>
            <p>看全国门店实时排队、营业状态和等待时间，先判断哪家店更值得去。</p>
            <div class="task-foot"><span class="mu">适合还没拿号的人</span><span class="task-arrow">›</span></div>
          </button>
          <button class="task-card" onclick="go('qd')" type="button">
            <span class="tag read">只读 · 不用凭证</span>
            <h3>我已经拿到号</h3>
            <p>输入门店和手里的号码，直接估算大概几点叫到、几点前到店。</p>
            <div class="task-foot"><span class="mu">适合看叫号进度</span><span class="task-arrow">›</span></div>
          </button>
          <button class="task-card" onclick="go('ca')" type="button">
            <span class="tag auth">需要凭证</span>
            <h3>想约未来某天</h3>
            <p>查预约日历、筛可预约时段，再按你的门店和时间偏好自动尝试。</p>
            <div class="task-foot"><span class="mu">适合抢周末或晚餐</span><span class="task-arrow">›</span></div>
          </button>
          <button class="task-card" onclick="go('sn')" type="button">
            <span class="tag action">会执行操作</span>
            <h3>自动蹲号</h3>
            <p>设置日期、门店和时间窗，到开放窗口自动高频尝试，抢到后停止。</p>
            <div class="task-foot"><span class="mu">适合明确目标时段</span><span class="task-arrow">›</span></div>
          </button>
        </div>
      ...
      <aside class="side">
        ...
        <details class="card adv">
          <summary>安全分级说明</summary>
          <div class="safety-grid" style="margin-top:12px">
            <div class="safety-card"><b><span class="safe-tag safe-read">只读</span>不会影响账号</b><p>看门店、看排队、看叫号预测，不需要你的账号凭证。</p></div>
            <div class="safety-card"><b><span class="safe-tag safe-auth">需要凭证</span>只读取状态</b><p>读取你的预约和排队信息，用于展示、提醒和恢复本地状态。</p></div>
            <div class="safety-card"><b><span class="safe-tag safe-action">会执行操作</span>必须确认</b><p>抢预约、远程取号、取消预约和取消排队号都会有明确确认。</p></div>
          </div>
        </details>
        <div class="card">
          <h2>第一次使用</h2>
          <div class="track">
            <div class="step" id="step-capture" style="cursor:pointer" onclick="startAuth()"><div class="mark">1</div><div><b>需要自动抢时再获取凭证</b><span>只看排队不用凭证；抢预约、取号、读取我的单据才需要。</span></div></div>
            <div class="step" id="step-prefs" style="cursor:pointer" onclick="go('se')"><div class="mark">2</div><div><b>选门店和偏好</b><span>设置人数、桌型、门店优先级和想要的时间段。</span></div></div>
            <div class="step" id="step-sampling" style="cursor:pointer" onclick="go('sm')"><div class="mark">3</div><div><b>开启信息收集</b><span>本机记录常用门店叫号变化，让到店判断更准。</span></div></div>
            <div class="step" id="step-booking" style="cursor:pointer" onclick="go('ca')"><div class="mark">4</div><div><b>查日历或自动蹲号</b><span>可约的时段可以直接预约；还没放出的时段再加入狙击蹲它。</span></div></div>
          </div>
          <div class="fl g8 fw mt16"><button class="bt bt-r bt-s" onclick="openFirstUseWizard()">打开新手向导</button><button class="bt bt-w bt-s" onclick="go('qt')">先看排队</button><button class="bt bt-w bt-s" onclick="go('se')">设置偏好</button></div>
        </div>
        <div class="card">
          <h2>当前偏好</h2>
  ...
  <section id="p-ca" class="hid">
    <div class="cd">
      <div class="fl ai jb mb16 fw g8">
        <div><h2 class="ph">预约未来</h2><p class="ph-sub">查询未来日期是否有可预约时段；需要自动创建预约时才会使用你的凭证。</p></div>
  ...
  <section id="p-in" class="hid">
    <div class="cd">
      <div class="page-lead"><h2 class="ph">历史洞察</h2><button class="bt bt-w bt-s" onclick="lI()">刷新</button></div>
  ...
  <section id="p-qd" class="hid">
    <div class="cd">
      <div class="dash-head">
        <div>
          <div class="cd-t" style="margin-bottom:8px">我有号码</div>
          <div class="dash-title">输入手里的号，判断几点到店</div>
          <p class="dash-copy"><span class="tag read">只读 · 不用凭证</span> 选门店、输入号码后，系统会直接估算大概几点叫到和建议到店时间；图表只是解释依据。</p>
        </div>
      ...
      <div id="qdPressureChart" class="dash-chart"><div class="empty">选门店并输入号码后，这里画今天的叫号进度和排队压力。</div></div>
      <div id="qdReminderCard" class="curve-sampling">
        <div>
          <b>多段到店提醒 <span class="tag read">只读提醒</span></b>
          <p>适合已经在小程序或现场取到号的人：按你手里的号提前多次提醒，不会替你取号或取消号。</p>
          <div class="fr mt8">
      ...
      <details class="card adv mt16">
        <summary><span class="setting-fold-title"><b>高级：模型曲线与历史依据</b><span>同星期历史叫号曲线、10 分钟叫号表、日期类型拆分；普通用户看上面的答案卡即可。</span></span></summary>
        <div id="qdAdvisor" class="advisor-panel mt16"><div class="ci">先选门店，再输入你手里的号；这里会显示大概几点叫到和几点前到店。</div></div>
        <div id="qdKpis" class="kpi-grid"><div class="ci">看板加载中</div></div>
        <div id="qdChart" class="dash-chart"><div class="empty">加载中</div></div>
        <div id="qdSamplingCard" class="curve-sampling"><div><b>本机持续采集</b><p>开启后会记录你关心门店的叫号、等位和可预约时段，曲线会越用越准；数据只留在本机，不会上传。需要先完成凭证。</p></div><div class="curve-sampling-actions"><button class="bt bt-w bt-s" onclick="go('sm')">详细配置</button></div></div>
        <div class="mt16">
  ...
  <section id="p-qp" class="hid">
    <div class="cd">
      <div class="dash-head">
        <div>
          <div class="cd-t" style="margin-bottom:8px">取号 → 几点能吃</div>
          <div class="dash-title">打算几点取号，预计几点吃上</div>
          <p class="dash-copy"><span class="tag read">只读 · 不用凭证</span> 用同门店同星期的历史等待，叠加今天的实时排队压力，估算就餐时间；只是参考，不会替你取号。</p>
        </div>
        <div class="dash-controls">
          <label class="dash-target">计划取号 <input id="qpPickup" type="time" value="12:10"></label>
          <button class="bt bt-w bt-s" onclick="openStorePicker({selected:qpStore?[qpStore]:[],multi:false,onConfirm:ids=>{qpStore=ids[0]||'';renderQpStore();loadQueuePickupPlan()}})">选门店</button>
  ...
  <section id="p-qw" class="hid">
    <div class="cd">
      <div class="dash-head">
        <div>
          <div class="cd-t" style="margin-bottom:8px">想几点吃 → 几点取号</div>
          <div class="dash-title">定个就餐时间，倒推取号窗口</div>
          <p class="dash-copy"><span class="tag read">只读 · 不用凭证</span> 按历史等待倒推建议取号时间，并提示最晚别拖过几点；叠加今天实时压力给风险等级。</p>
        </div>
        <div class="dash-controls">
          ...
          <button class="bt bt-w bt-s" onclick="openStorePicker({selected:qwStore?[qwStore]:[],multi:false,onConfirm:ids=>{qwStore=ids[0]||'';renderQwStore();loadQueueMealPlan()}})">选门店</button>
  ...
  <section id="p-qt" class="hid">
    <div class="cd">
      <div class="page-lead"><div><h2 class="ph">现在去吃</h2><p class="ph-sub"><span class="tag read">只读 · 不用凭证</span> 先看门店是否营业、前面还有几桌、大概等多久；只有点击远程取号或启用自动取号时才会执行操作。</p></div><div class="fl g8 fw"><button class="bt bt-w bt-s" onclick="refreshQueueView()">刷新</button></div></div>
      ...
      <summary>高级 · 取号计划 / 基准采集 / 叫号提醒</summary>
      <div class="qbox mt16">
      ...
      <div class="qbox mt16">
        <div class="fl ai jb fw g8"><label style="margin:0">全国门店基准采集 <span class="tag read">只读</span></label><span class="mu">周期快照全部门店等位/状态写本地作为基准；走公开接口，无需凭证。</span></div>
      ...
      <div class="mt16" id="qtAlertCard">
        ...
        <div class="fl g8 fw mt8">
          ...
          <div class="fg hid" id="qaLabelWrap"><label>标签</label><input id="qaLabel" placeholder="我 / 朋友"></div>
          <div class="fg hid" id="qaTravelWrap"><label>路程(分钟)</label><input type="number" id="qaTravel" min="0"></div>
          <div class="fg hid" id="qaNotifyWrap"><label>提醒点(可多个)</label><input type="text" id="qaNotifyAt" placeholder="如 1000,1025,1050"></div>
          <div class="fg hid" id="qaLeadWrap"><label>或提前(桌)</label><input type="number" id="qaLead" value="5"></div>
  ...
  <section id="p-sm" class="hid">
    <div class="cd">
      <div class="page-lead"><div><h2 class="ph">本机数据收集</h2><p class="ph-sub"><span class="tag auth">需要凭证</span> 只记录你关心门店的预约和排队变化，用来提升“几点叫到”的判断；不会自动取消或创建预约。</p></div><div class="fl g8 fw"><button class="bt bt-w bt-s" onclick="runSampleOnce()">立即收集一次</button><button class="bt bt-r bt-s" onclick="startSampling()">开始信息收集</button></div></div>
      <details class="btn-more mb16"><summary></summary><div class="fl g8 fw"><button class="bt bt-w bt-s" onclick="setBootSampling(true)">启用开机自启动</button><button class="bt bt-o bt-s" onclick="setBootSampling(false)">取消开机自启动</button><button class="bt bt-o bt-s" onclick="stopSampling()">暂停运行</button></div></details>
  ...
  <section id="p-re" class="hid">
    <div class="cd"><h2 class="ph">我的预约 / 排队号</h2><p class="ph-sub mb16"><span class="tag auth">需要凭证</span> 这里用于查看官方预约和排队号；取消按钮是危险操作，会单独确认。</p><div id="rc"><div class="empty">加载中</div></div></div>
  </section>

  <section id="p-se" class="hid">
    <div class="settings-grid">
      <div class="section-banner"><b>常用设置</b><span>普通用户只需要设置门店、人数、时间偏好和通知。凭证、诊断、接口调试都放在下方高级工具区。</span></div>
      <div class="cd">
        ...
        <div class="fg"><label>抢号门店与优先级</label><div id="bookingStores" class="store-list"><span class="mu">用上方搜索添加，或完成凭证后自动带入</span></div><div class="ps mt8">抢预约 / 取号会按勾选门店的排序依次尝试。新加的门店若从没在小程序点过，建议刷新凭证后先试一家确认可用。</div></div>
        <div class="fr mb16">
      ...
      <div class="section-banner"><b>高级工具</b><span>只有抢预约、远程取号、状态异常或开发排查时才需要使用。只看排队和叫号预测不需要操作这里。</span></div>
      <details class="cd setting-fold">
        <summary><span class="setting-fold-title"><b>手机凭证捕获</b><span>手机代理抓取真实微信凭证参数；只在需要抢预约、读我的单据时使用。</span></span></summary>
        <div class="setting-fold-body">
        <div class="fl ai jb mb16 fw g8"><div class="cd-t" style="margin-bottom:0">手机凭证捕获</div><div class="fl g8 fw"><button class="bt bt-r bt-s" onclick="openAuthWizard()">获取凭证向导</button><button class="bt bt-w bt-s" onclick="startMobileAuthCapture()">启动手机捕获</button><button class="bt bt-w bt-s" onclick="loadMobileAuth()">刷新</button><button class="bt bt-o bt-s" onclick="stopMobileAuthCapture()">停止</button></div></div>
        <div class="ps">用于 Windows 端：手机真实微信产生寿司郎凭证流量，电脑只捕获凭证参数。不会修改 Windows 系统代理；捕获完成后请关闭手机 Wi-Fi 代理。</div>
      ...
      <details class="cd setting-fold">
        <summary><span class="setting-fold-title"><b>手动导入凭证</b><span>手机抓包后粘贴 JSON、curl 或原始请求头；适合路由器隔离时使用。</span></span></summary>
        <div class="setting-fold-body">
        <div class="fl ai jb mb16 fw g8"><div class="cd-t" style="margin-bottom:0">手动导入凭证</div><div class="fl g8 fw"><button class="bt bt-r bt-s" onclick="importAuthText()">解析并保存</button><button class="bt bt-w bt-s" onclick="el('authImportText').value=''">清空</button></div></div>
        <div class="ps">网络隔离导致手机连不上电脑代理时使用：在手机本机抓包工具里复制寿司郎请求的 JSON、curl 或 raw headers，粘贴到这里。建议至少包含一次门店/查询请求和一次预约或排队请求。</div>
        <div class="fg mt8"><label>凭证内容</label><textarea id="authImportText" placeholder="支持 JSON / curl / raw headers。字段包括 X-App-Code、Authorization、User-Agent、Referer、wechatId、phoneNumber、storeId。"></textarea></div>
        <div class="fl g8 fw"><button class="bt bt-r bt-s" onclick="importAuthText()">解析并保存</button><button class="bt bt-w bt-s" onclick="testAuthProbe()">保存后测试基础接口</button></div>
        <div id="authImportState" class="diag-detail mt8">尚未导入</div>
        </div>
      </details>
      <details class="cd setting-fold">
        <summary><span class="setting-fold-title"><b>移动端 UA</b><span>采集或手动保存手机微信 UA；通常不用展开。</span></span></summary>
        <div class="setting-fold-body">
      ...
      <details class="cd setting-fold">
        <summary><span class="setting-fold-title"><b>接口调试</b><span>只给开发排查官方接口变化用；捕获记录默认折叠，不打扰普通用户。</span></span></summary>
        <div class="setting-fold-body">
        ...
        <div class="ps mt8">开启后，在“获取凭证”捕获期间打开 PC 微信寿司郎小程序并点击“我的预约/预约详情/取消预约”等页面。后端只记录接口路径、状态码、query key、请求 body key 和响应 JSON 结构，不记录 token、手机号、微信 ID 或原始 body。</div>
        <div id="discState" class="diag-detail mt8">尚未加载</div>
        <div id="discRecords" class="mt8"><span class="mu">暂无调试记录</span></div>
        </div>
      </details>
      <details class="cd setting-fold">
        <summary><span class="setting-fold-title"><b>本机诊断和日志</b><span>状态异常时再打开。代理、证书、端口、捕获日志都在这里。</span></span></summary>
        <div class="setting-fold-body">
        <div class="fl ai jb mb16 fw g8"><div class="cd-t" style="margin-bottom:0">本机诊断</div><div class="fl g8 fw"><button class="bt bt-w bt-s" onclick="lD()">刷新</button><button class="bt bt-w bt-s" onclick="testAuthProbe()">测试基础接口</button><button class="bt bt-w bt-s" onclick="copyDiag()">复制诊断</button><button class="bt bt-r bt-s" onclick="repairP()">修复代理</button></div></div>
  ...
  <section id="p-lo" class="hid">
    <div class="cd"><h2 class="ph mb16">运行日志</h2><div class="lg" id="lv"></div></div>
  </section>
</main>
<footer class="ft">由 <a href="https://github.com/Ryujoxys/sushiro-overdose">sushiro-overdose</a> 驱动 · 非官方工具，仅供学习</footer>

<script>
let cp='da',es={status:'idle'},hc=0,as=[],sd='',pr={},pf='',cE=null,stores=[],selStores=[],calErrs=[],arTimer=null,lastDiag=null,spCfg={},spState={status:'idle'},spAutoStart={},spQueueState={},qdSelected=[],qtSelected=[],qtTrendStores=[],qaStatus={},ah={},nfc=true,qpStore='',qwStore='';
...
const NAV_GROUPS=[
  ...
  {id:'book',label:'预约未来',pages:[['ca','预约日历'],['in','历史洞察']]},
  {id:'auto',label:'自动蹲号',pages:[['sn','狙击计划']]},
  {id:'mine',label:'我的单据',pages:[['re','预约 / 排队号']]},
  {id:'settings',label:'设置',pages:[['se','设置'],['sm','数据收集'],['lo','日志']]}
];
...
function go(n,e){if(!PAGE_GROUP[n])n='da';document.querySelectorAll('.wrap>section[id^="p-"]').forEach(p=>p.classList.add('hid'));const sec=el('p-'+n);if(sec)sec.classList.remove('hid');const gid=PAGE_GROUP[n]||'home',g=NAV_GROUPS.find(x=>x.id===gid);document.querySelectorAll('.nav.top a').forEach(a=>a.classList.toggle('on',a.dataset.group===gid));renderSubnav(g,n);cp=n;if(location.hash.slice(1)!==n)history.replaceState(null,'','#'+n);({ca:lC,in:lI,qd:lQD,qp:lQP,qw:lQW,qt:lQT,sm:lSm,sn:lSn,re:lR,se:lS,lo:lL})[n]?.();return false}
async function loadStatus(){try{const r=await(await fetch('/api/status')).json();el('ver').textContent='v'+r.version;hc=!!r.has_config;pf=r.platform||'';es=r.engine||{status:'idle'};spState=r.sampling||spState;ah=r.auth_health||{};nfc=r.notify_configured!==false;uE();uSamplingSummary();uD();uAuth();}catch(e){el('ver').textContent='offline';}}
function uAuth(){const pill=el('authPill'),banner=el('authBanner'),st=(ah&&ah.status)||'unknown';if(pill){if(!hc){pill.className='authpill';pill.textContent='只读模式'}else if(st==='stale'){pill.className='authpill stale';pill.textContent='凭证可能过期'}else{pill.className='authpill ok';pill.textContent='已凭证'}pill.classList.remove('hid')}if(banner){if(hc&&st==='stale'){banner.classList.remove('hid');banner.innerHTML='<b>凭证可能已过期</b>在手机上用过寿司郎小程序后，电脑这边的凭证就会失效（同一账号只认一个会话）。<button class="bt bt-r bt-s" onclick="event.stopPropagation();startAuth()">重新获取凭证</button>'}else{banner.classList.add('hid');banner.innerHTML=''}}}
function authPillClick(){if(!hc||(ah&&ah.status==='stale'))startAuth();else go('re')}
async function init(){await loadStatus();await lP();checkUpdate();sse();const h=location.hash.slice(1);if(h&&PAGE_GROUP[h]&&h!=='da')go(h);else maybeShowIntro();}
...
function uD(){
  const b=el('bm'),bc=el('bc'),brst=el('brst'),nc=el('nc'),title=el('heroTitle'),copy=el('heroCopy'),badge=el('heroBadge');
  const run=isRun();
  b.disabled=run;
  ...
  if(es.status==='capturing'){
    badge.textContent='正在获取凭证';title.textContent='按引导操作一次小程序';copy.textContent='只需要点进寿司郎小程序产生一次请求，不要提交预约，也不要取消任何订单。抓到字段后页面会自动更新。';
    b.textContent='获取中';b.className='bt bt-y bt-l';b.onclick=sC;
    bc.classList.add('hid');
    brst.classList.remove('hid');
    ...
    b.textContent=hc?'查看预约日历':'先看实时排队';b.className='bt bt-y bt-l';b.onclick=hc?(()=>go('ca')):(()=>go('qt'));
    bc.textContent=hc?'重新获取凭证':'获取凭证';
    bc.onclick=startAuth;
    ...
    badge.textContent='第一次使用';title.textContent='先不用凭证，直接看排队';copy.textContent='只看全国门店、实时排队和叫号预测不需要账号凭证。只有抢未来预约、远程取号、读取我的单据时才需要获取凭证。';
    b.textContent='现在去吃：看排队';b.className='bt bt-r bt-l';b.onclick=()=>go('qt');
    bc.textContent=pf==='windows'?'我要抢预约，用手机获取凭证':'我要抢预约，获取凭证';
    bc.onclick=startAuth;
    nc.classList.remove('hid');nc.textContent='建议先从“现在去吃”或“我有号码”开始；确认要自动操作时再做凭证。';
  }else{
    const hasStores=(pr.selected_stores||[]).length>0;
    if(!hasStores){
      badge.textContent='凭证已保存';title.textContent='下一步：选择门店和偏好';copy.textContent='抢未来预约前，需要先选门店、人数、桌型和时间偏好。只看排队仍然可以直接使用。';
      b.textContent='设置门店和偏好';b.className='bt bt-y bt-l';b.onclick=()=>go('se');
      bc.textContent='先看实时排队';bc.onclick=()=>go('qt');
    }else{
      badge.textContent='准备就绪';title.textContent='可以查日历，也可以自动蹲号';copy.textContent='已保存凭证和门店偏好。建议先查看预约日历；如果目标明确，再启动自动蹲号或自动抢预约。';
      b.textContent='查看预约日历';b.className='bt bt-r bt-l';b.onclick=()=>go('ca');
      bc.textContent='开始自动抢预约';
      bc.className='bt bt-o';
      bc.onclick=sB;
      brst.textContent='重新获取凭证';
      brst.onclick=startAuth;
      brst.classList.remove('hid');
    }
  }
}
function uE(){
  ...
  const label={idle:'就绪',capturing:'正在获取凭证',booking:'正在抢号',sniping:'狙击中',success:'预约成功',error:'需要处理'}[s.status]||s.status;
...
function openFirstUseWizard(){let ov=el('firstUse');if(!ov){ov=document.createElement('div');ov.id='firstUse';ov.className='ov';document.body.appendChild(ov)}ov.innerHTML='<div class="ovc" style="width:min(820px,96vw)"><div class="fl ai jb mb16"><div><b>你现在想做什么？</b><div class="mu mt8">先选场景；只读功能会直接进入，需要凭证或会执行操作的功能会先提示。</div></div><button class="bt bt-w bt-s" onclick="closeFirstUseWizard()">关闭</button></div><div class="task-grid"><button class="task-card" type="button" onclick="firstUseGo(\'qt\',false)"><span class="tag read">只读</span><h3>现在想去吃</h3><p>看门店营业、等位和实时叫号，不需要凭证。</p><div class="task-foot"><span class="mu">直接进入</span><span class="task-arrow">›</span></div></button><button class="task-card" type="button" onclick="firstUseGo(\'qd\',false)"><span class="tag read">只读</span><h3>我已经拿到号</h3><p>输入号码，判断大概几点叫到、几点前到店。</p><div class="task-foot"><span class="mu">直接进入</span><span class="task-arrow">›</span></div></button><button class="task-card" type="button" onclick="firstUseGo(\'ca\',true)"><span class="tag auth">需要凭证</span><h3>想约未来某天</h3><p>查预约日历，准备按偏好自动抢预约。</p><div class="task-foot"><span class="mu">没有凭证会先引导获取</span><span class="task-arrow">›</span></div></button><button class="task-card" type="button" onclick="firstUseGo(\'sn\',true)"><span class="tag action">会执行操作</span><h3>自动蹲号</h3><p>设置目标时段，到窗口自动尝试创建预约。</p><div class="task-foot"><span class="mu">启动前还会再次确认</span><span class="task-arrow">›</span></div></button></div></div>';ov.classList.remove('hid');ov.style.display='flex'}
function closeFirstUseWizard(){const ov=el('firstUse');if(ov){ov.classList.add('hid');ov.style.display='none'}}
async function firstUseGo(page,needsAuth){closeFirstUseWizard();if(needsAuth&&!hc){if(await confirmDialog({title:'需要先获取凭证',body:'这个功能需要先获取凭证。\\n只看排队和叫号预测不用凭证；抢预约或自动蹲号才需要。\\n现在去获取凭证？',ok:'去获取凭证',cancel:'先看看'}))startAuth();else go(page);return}go(page)}
...
function authWizShell(body){return '<div class="ovc"><div class="fl ai jb mb16"><b>用手机获取凭证</b><button class="bt bt-w bt-s" onclick="closeAuthWizard()">关闭</button></div><div style="overflow:auto">'+body+'</div></div>'}
function authWizStep(step){const ov=el('authWiz');if(!ov)return;if(authWizPoll){clearInterval(authWizPoll);authWizPoll=null}
  if(step==='home'){ov.innerHTML=authWizShell('<p class="mu">Windows 上 PC 微信抓不到凭证，需要用手机拿一次。整个过程就三步：</p><ol class="mu" style="margin:8px 0 0;padding-left:20px"><li><b>手机上</b>用抓包工具或扫码，让寿司郎小程序产生一次请求</li><li><b>把抓到的内容发到电脑</b>（推荐微信「文件传输助手」）</li><li><b>粘贴到这里</b>，自动解析保存</li></ol><p class="mu mt16">先选你的手机：</p><div class="fl g8 fw mt8"><button class="bt bt-r" onclick="authWizStep(\'ios\')">iPhone</button><button class="bt bt-r" onclick="authWizStep(\'android\')">安卓</button></div>')}
  else if(step==='ios'){ov.innerHTML=authWizShell('<p class="mu">iPhone 两种方式，<b>推荐「手动导入」</b>——在手机本机抓，不依赖路由器，遇到 AP（客户端）隔离也能用；自动代理抓只在手机和电脑同一 Wi-Fi 且无 AP 隔离时可用。</p><div class="fl g8 fw mt16"><button class="bt bt-r" onclick="authWizStep(\'ios_manual\')">手动导入（推荐·最稳）</button><button class="bt bt-w" onclick="authWizStep(\'ios_auto\')">自动代理抓</button></div><div class="mt16"><button class="bt bt-w bt-s" onclick="authWizStep(\'home\')">← 返回</button></div>')}
...
async function authWizPollAuto(){try{const d=await safeFetch('/api/mobile-auth');authWizRenderAuto(d);if(d.saved||d.config_complete){if(authWizPoll){clearInterval(authWizPoll);authWizPoll=null}await loadStatus();toast('凭证已获取并保存！请把手机 Wi-Fi 代理改回关闭。');closeAuthWizard()}}catch(e){}}
function authWizRenderAuto(d){const b=el('awAuto');if(!b)return;const urls=d.guide_urls||[],hosts=d.hosts||[];b.innerHTML='<p class="mu">用 iPhone 微信「扫一扫」下面二维码打开引导页：①装 CA 证书 ②设置→通用→关于本机→证书信任设置里完全信任 ③把 Wi-Fi 的 HTTP 代理设为下面的 电脑IP:端口 ④彻底关掉再打开微信，进寿司郎小程序点一次门店/排队/预约。抓到后这里会自动提示完成。</p><div class="mt8" style="text-align:center">'+((d.active&&d.qr_svg)?d.qr_svg:'<span class="mu">二维码加载中…</span>')+'</div><div class="ps mt8">'+(urls.length?'<b>扫码或手机浏览器打开：</b><br>'+urls.map(u=>'<code>'+esc(u)+'</code>').join('<br>'):'')+'<div class="mu mt8"><b>Wi-Fi 代理：</b>'+hosts.map(h=>'<code>'+esc(h)+':'+esc(d.proxy_port||'')+'</code>').join(' ')+'</div><div class="mu mt8">扫码打不开 / 手机连不上电脑？多半是路由器开了 AP（客户端）隔离。<button class="bt bt-w bt-s" onclick="authWizStep(\'ios_manual\')">改用手动导入（更稳）</button></div></div><div class="diag-detail mt8">'+esc(d.message||'')+'</div>'}
function authWizImportHTML(platform){const guide=platform==='android'?'<b>安卓怎么抓：</b><ol><li>手机装一个能抓 HTTPS 的工具（如 Reqable / Stream / HttpCanary），按它的说明装好并信任证书。</li><li>开启抓包，打开微信寿司郎小程序，点一次门店列表/排队/预约，再点一次「我的预约」。</li><li>在工具里找到 <code>crm-cn-prd.sushiro.com.cn</code> 的请求，导出/复制成 cURL 或原始请求头。</li><li><b>把内容发到电脑</b>（见下方），在电脑上粘贴到框里，点「解析并保存」。</li></ol>':'<b>iPhone 手动抓（本机，不依赖路由器，最稳）：</b><ol><li>App Store 装一个本机抓 HTTPS 的工具（如 Stream），按其说明安装并信任证书。</li><li>开启抓包，打开微信寿司郎小程序，点一次门店/排队/预约，再点一次「我的预约」。</li><li>找到 <code>crm-cn-prd.sushiro.com.cn</code> 的请求，导出/复制成 cURL 或原始请求头。</li><li><b>把内容发到电脑</b>（见下方），在电脑上粘贴到框里，点「解析并保存」。</li></ol>';const transfer='<div class="warn mt8" style="background:#fff6d8;border:1px solid #f2cf6a;border-radius:12px;padding:10px"><b>📲 怎么把手机抓到的内容弄到电脑</b><div class="mu mt8">最稳：手机微信里搜「<b>文件传输助手</b>」，把复制的内容发给它 → 电脑微信打开同一会话 → 复制 → 粘贴到下面。手机电脑不在同一网络也能用。</div></div>';return '<div class="ps">'+guide+'</div>'+transfer+'<div class="fg mt8"><label>粘贴抓包内容（JSON / cURL / 原始请求头）</label><textarea id="awImport" placeholder="包含 X-App-Code、Authorization、User-Agent、Referer、wechatId、phoneNumber、storeId…"></textarea></div><div id="awImportState" class="diag-detail mt8 hid"></div><div class="fl ai jb mt16"><button class="bt bt-w bt-s" onclick="authWizStep(\''+(platform==='android'?'home':'ios')+'\')">← 返回</button><button class="bt bt-r" onclick="authWizImport()">解析并保存</button></div>'}
async function authWizImport(){const txt=(el('awImport')?.value||'').trim();if(!txt){toast('请先粘贴抓包内容');return}const st=el('awImportState');if(st){st.classList.remove('hid');st.innerHTML='解析中…'}try{const d=await safeFetch('/api/auth/import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:txt})},15000);if(d.saved){if(st)st.innerHTML='<span class="ok">已保存</span> '+esc(d.message||'');await loadStatus();toast('凭证已导入并保存！');closeAuthWizard()}else{const miss=d.missing||[],need=[];if(miss.some(x=>/预约|微信|手机/.test(x)))need.push('再抓一次『我的预约 / 预约』请求');if(miss.some(x=>/查询|Referer|门店/i.test(x)))need.push('再抓一次『门店 / 排队』请求');if(st)st.innerHTML='<span class="bad">字段不完整，还缺：</span>'+esc(miss.join('、')||'未知')+(need.length?'<br><span class="mu">'+esc(need.join('；'))+'，把内容一起粘进来即可。</span>':'<br><span class="mu">再补一段包含缺失字段的请求即可。</span>')}}catch(e){if(st)st.innerHTML='<span class="bad">导入失败：'+esc(String(e.message||e))+'</span>'}}

async function lC(){await ensureStores();if(!stores.length){el('storeChoices').innerHTML='<span class="mu">预约未来需要先获取凭证；只看排队不用。</span>';el('sc').innerHTML='<div class="empty">想查看未来可预约时段，需要先获取凭证。只看实时排队请去“现在去吃”。<div class="mt8"><button class="bt bt-r bt-s" onclick="startAuth()">获取凭证</button><button class="bt bt-w bt-s" onclick="go(\'qt\')">先看排队</button></div></div>';return}if(!selStores.length)selStores=stores.map(s=>String(s.id));rStoreChoices();rC()}
...
function calendarErrHTML(){return calErrs.length?'<div class="errbox">'+calErrs.map(x=>'<b>'+esc(x.store)+'</b>：'+esc(x.error)).join('<br>')+'<div class="mt8"><button class="bt bt-o bt-s" onclick="sC()">重新获取凭证</button></div></div>':''}
function rDB(){const g={};as.forEach(s=>{if(!g[s.date])g[s.date]=[];g[s.date].push(s)});const ds=Object.keys(g).sort(),b=el('dbar');b.innerHTML='';if(!ds.length){el('sc').innerHTML=calendarErrHTML()+'<div class="empty">这几家门店当前没有返回可展示时段。可以刷新，或换一家门店试试。</div>';return}if(!sd||!ds.includes(sd))sd=ds[0];ds.forEach(d=>{const sl=g[d],av=sl.filter(s=>s.availability==='AVAILABLE').length,dt=new Date(d.substring(0,4)+'-'+d.substring(4,6)+'-'+d.substring(6,8)),c=document.createElement('div');c.className='dc'+(d===sd?' on':'');c.innerHTML='<div class="dw">周'+W[dt.getDay()]+'</div><div class="dd">'+fD(d)+'</div><div class="dv '+(av>0?'h':'n')+'">'+(av>0?'可约 '+av:'已满')+'</div>';c.onclick=()=>{sd=d;rDB()};b.appendChild(c)});rS(sd)}
...
function applyDashboardStores(ids){qdSelected=(ids||[]).slice(0,1).map(String);if(qdSelected.length&&el('qdScope'))el('qdScope').value='local';renderDashboardStores();renderReminderTemplateHint();loadQueueDashboard();loadQueueAlertStatus()}
function renderDashboardStores(){const c=el('qdStores');if(!c)return;if(!qdSelected.length){c.innerHTML='<span class="mu">未指定门店：主图会自动选择本机叫号样本最多、最新的门店。</span>';renderTicketReminderCard();return}c.innerHTML=qdSelected.map(id=>'<button class="chip on" data-store="'+escA(String(id))+'">'+esc(storeDisplayName(id))+' ✕</button>').join('')+'<button class="chip" onclick="qdSelected=[];renderDashboardStores();renderReminderTemplateHint();loadQueueDashboard();loadQueueAlertStatus()">清空</button>';c.querySelectorAll('.chip.on').forEach(b=>b.onclick=()=>{const id=b.dataset.store;qdSelected=qdSelected.filter(x=>x!==id);renderDashboardStores();renderReminderTemplateHint();loadQueueDashboard();loadQueueAlertStatus()})}
...
function renderReminderTemplateHint(){const target=parseInt(el('qdTargetNo')?.value||'',10),input=el('qdrPoints'),tpl=el('qdrTemplate')?.value||'normal';if(input&&!(input.value||'').trim()){const pts=target>0?reminderTemplatePoints(target,tpl):[];input.placeholder=pts.length?'默认 '+pts.join(','):'如 1000,1025,1050'}renderTicketReminderCard()}
...
function renderTicketReminderCard(err){const box=el('qdReminderStatus');if(!box)return;if(err){box.innerHTML='<div class="ci bad">'+esc(err)+'</div>';return}const s=qdReminderStore(),target=parseInt(el('qdTargetNo')?.value||'',10),points=target>0?reminderPointsFromInputs(target):[],n=qaStatus.notifications||{},sampling=qaStatus.sampling||{},channels=(n.channels||[]).join('、')||'未配置',notifyClass=n.configured?'ok':'bad',sampleClass=sampling.running||sampling.daemon_running||sampling.system_auto_start?.enabled?'ok':'warn',hint=!s?'请先选一个门店；提醒不会自动取号。':!target?'请输入手里的号；提醒点会自动按模板生成。':points.length?('将为 '+esc(s.name)+' · '+fmtN(target)+'号 设置提醒点：'+points.map(fmtN).join('、')):'请填写有效提醒点（不能大于手里的号）。';const chips=chip('通知',channels,notifyClass)+chip('采集',sampling.running?'运行中':sampling.daemon_running?'后台运行':sampling.system_auto_start?.enabled?'已设开机采集':(sampling.message||'未持续采集'),sampleClass);const rules=(qaStatus.rules||[]).filter(x=>x.rule&&x.rule.type==='called_reach'&&(!s||String(x.rule.store_id)===s.id)&&(!target||x.rule.target_no===target));const rows=rules.length?'<div class="sg mt8">'+rules.map(x=>{const r=x.rule||{},cls=x.status==='fired'?'av':x.status==='due'?'fu':'av',eta=x.estimate_to_threshold_minutes!=null?(' · 预计 '+fmtN(x.estimate_to_threshold_minutes)+' 分钟到提醒点'):'',next=x.next?' · 下一条':'';return'<div class="sl '+cls+'"><div class="ss">'+esc(x.label||r.label||((r.target_no||0)+'号'))+' · '+fmtN(r.target_no||0)+'号</div><div class="mu mt8">到/过 '+fmtN(x.threshold||qaRuleThreshold(r))+' 号提醒 · 当前 '+fmtN(x.current_called_no||0)+' · 还差 '+fmtN(x.remaining_to_threshold||0)+' 桌'+eta+next+'</div><div class="mu mt8">'+esc(x.status_text||'监控中')+(r.travel_minutes?(' · 路程约 '+fmtN(r.travel_minutes)+' 分钟'):'')+'</div></div>'}).join('')+'</div>':'<div class="mu mt8">还没有匹配当前门店和号码的提醒。生成后会显示每一段的进度。</div>';box.innerHTML='<div>'+chips+'</div><div class="mu mt8">'+hint+'</div>'+rows}
async function createTicketReminder(){const s=qdReminderStore();if(!s){toast('请先选门店');return}const target=parseInt(el('qdTargetNo')?.value||'',10);if(!target){toast('请填写你手里的号');return}const points=reminderPointsFromInputs(target);if(!points.length){toast('请填写有效提醒点，且不能大于手里的号');return}const label=(el('qdrLabel')?.value||'').trim(),travel=Math.max(0,parseInt(el('qdrTravel')?.value||'',10)||0),tpl=el('qdrTemplate')?.value||'normal';try{let base=qtAlerts||[];try{const d=await safeFetch('/api/queue/alerts');base=(d&&d.rules)||base}catch(e){}const rules=base.slice();points.forEach(n=>rules.push({store_id:s.id,store_name:s.name,label:label,type:'called_reach',target_no:target,notify_at_no:n,lead_groups:Math.max(0,target-n),travel_minutes:travel,template:tpl,enabled:true}));const saved=await safeFetch('/api/queue/alerts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({rules:rules})});qtAlerts=(saved&&saved.rules)||rules;renderQueueAlerts();await loadQueueAlertStatus();toast('已生成 '+points.length+' 个提醒点')}catch(e){toast('生成提醒失败：'+String(e.message||e))}}
...
function renderPressureChart(box,curve,adv,target){if(!box)return;const points=(curve&&curve.points||[]).filter(p=>hhmmMinute(p.time)!=null).slice().sort((a,b)=>hhmmMinute(a.time)-hhmmMinute(b.time));if(!points.length){box.innerHTML='<div class="empty">'+esc((curve&&curve.message)||'还没有今天的本机采样曲线。开启「本机持续采集」后会逐步补齐；现在可看上面的实时答案卡。')+'</div>';return}const w=1040,h=286,l=48,r=44,t=24,b=40,minM=600,maxM=1320,maxCalled=Math.max(10,...points.map(p=>p.called_no||0)),x=m=>l+(Math.min(maxM,Math.max(minM,m))-minM)/(maxM-minM)*(w-l-r),yCall=v=>h-b-(v/maxCalled)*(h-t-b),yPress=s=>h-b-(Math.min(100,Math.max(0,s))/100)*(h-t-b);let svg='<svg viewBox="0 0 '+w+' '+h+'" preserveAspectRatio="none">';for(let i=0;i<=4;i++){const yy=t+i*(h-t-b)/4,val=Math.round(maxCalled*(4-i)/4);svg+='<line class="chart-grid" x1="'+l+'" y1="'+yy+'" x2="'+(w-r)+'" y2="'+yy+'"></line><text class="chart-label" x="4" y="'+(yy+4)+'">'+fmtN(val)+(i===0?' 号':'')+'</text>'}for(let hh=10;hh<=22;hh+=2){const xx=x(hh*60);svg+='<line class="chart-grid" x1="'+xx+'" y1="'+t+'" x2="'+xx+'" y2="'+(h-b)+'" opacity=".55"></line><text class="chart-label" x="'+xx+'" y="'+(h-9)+'" text-anchor="middle">'+(hh<10?'0':'')+hh+':00</text>'}svg+='<line class="chart-axis" x1="'+l+'" y1="'+(h-b)+'" x2="'+(w-r)+'" y2="'+(h-b)+'"></line>';const pressArea=points.map(p=>x(hhmmMinute(p.time))+','+yPress(p.pressure_score||0));if(pressArea.length){const base=(h-b);svg+='<polygon points="'+l+','+base+' '+pressArea.join(' ')+' '+(w-r)+','+base+'" fill="rgba(120,120,120,.18)" stroke="rgba(120,120,120,.5)" stroke-width="1"></polygon>'}const callPts=points.filter(p=>(p.called_no||0)>0);let stepPath='';callPts.forEach((p,i)=>{const cx=x(hhmmMinute(p.time)),cy=yCall(p.called_no);stepPath+=(i===0?'M':'L')+cx+','+cy+' ';if(i<callPts.length-1){const nx=x(hhmmMinute(callPts[i+1].time));stepPath+='L'+nx+','+cy+' '}});if(stepPath)svg+='<path d="'+stepPath+'" fill="none" stroke="var(--red)" stroke-width="3" stroke-linejoin="round" stroke-linecap="round"></path>';const nowMin=(()=>{const dd=new Date();return dd.getHours()*60+dd.getMinutes()})();if(nowMin>=minM&&nowMin<=maxM){const nx=x(nowMin);svg+='<line x1="'+nx+'" y1="'+t+'" x2="'+nx+'" y2="'+(h-b)+'" stroke="var(--red)" stroke-width="1.4" opacity=".8"></line><text class="chart-label" x="'+(nx+4)+'" y="'+(t+10)+'" fill="var(--red)">现在</text>'}if(target>0&&target<=maxCalled){const my=yCall(target);svg+='<line x1="'+l+'" y1="'+my+'" x2="'+(w-r)+'" y2="'+my+'" stroke="var(--red)" stroke-width="1.4" stroke-dasharray="4 4" opacity=".9"></line><text class="chart-label" x="'+(w-r-4)+'" y="'+(my-4)+'" text-anchor="end" fill="var(--red)">我的号 '+fmtN(target)+'</text>'}const etaTip=(adv&&adv.eta&&adv.eta.estimated_called_at_range)?('\n预计叫到你：'+shortTime(adv.eta.estimated_called_at_range.early)+'-'+shortTime(adv.eta.estimated_called_at_range.late)):'';callPts.forEach((p,i)=>{const cx=x(hhmmMinute(p.time)),cy=yCall(p.called_no),s15=p.called_speed_15!=null?(Math.round(p.called_speed_15*15)+' 桌'):'数据不足',tip=p.time+'\n当前叫到：'+fmtN(p.called_no)+'\n排队压力：'+pressureLabelCN(p.pressure_level)+'\n等待桌数：'+fmtN(p.waiting_groups||0)+'\n官方等待：'+fmtN(p.official_wait_minutes||0)+' 分\n近15分钟叫号：'+s15+(i===callPts.length-1?etaTip:'');svg+='<g class="chart-hot" data-tip="'+escA(tip)+'" onmousemove="dashTip(event,this)" onmouseleave="hideDashTip()"><circle cx="'+cx+'" cy="'+cy+'" r="'+(i===callPts.length-1?5:3.5)+'" fill="'+(i===callPts.length-1?'#B81C22':'#fff')+'" stroke="#B81C22" stroke-width="2"></circle></g>'});svg+='</svg>';box.innerHTML=svg+'<div class="chart-legend"><span class="legend-line">叫号进度</span><span class="legend-pressure">排队压力</span><span class="legend-now">现在</span><span class="legend-mine">我的号</span><span class="mu">只展示 10:00-22:00</span></div>'}
...
// ---------- 取号→几点吃 ----------
async function lQP(){await ensureStores();if(!qpStore&&qdSelected[0])qpStore=qdSelected[0];renderQpStore();if(qpStore)loadQueuePickupPlan()}
...
async function renderPickupChart(box){if(!box||!qpStore){return}box.innerHTML='<div class="empty">不同取号时间对应的预计等待（历史 P50/P80）。</div>';try{const tr=await safeFetch('/api/queue/trends?store='+encodeURIComponent(qpStore),null,15000);const series=(tr.series||[]).filter(p=>String(p.store_id)===String(qpStore)&&p.wait_p50_minutes!=null).sort((a,b)=>hhmmMinute(a.bucket)-hhmmMinute(b.bucket));if(!series.length){box.innerHTML='<div class="empty">这家店历史等待样本还不足，先开启本机数据收集积累几次。</div>';return}drawWaitByPickup(box,series,(el('qpPickup')?.value||'').replace(':',''))}catch(e){box.innerHTML='<div class="empty">历史等待曲线暂不可用。</div>'}}
...
// ---------- 想几点吃→几点取号 ----------
async function lQW(){await ensureStores();if(!qwStore&&qdSelected[0])qwStore=qdSelected[0];renderQwStore();if(qwStore)loadQueueMealPlan()}
...
function renderDashboardCalledCurve(points,summary,advisor){const box=el('qdChart');advisor=advisor||{};if(!box)return;points=(points||[]).slice().filter(p=>bucketMinute(p.bucket)!=null&&bucketMinute(p.bucket)>=600&&bucketMinute(p.bucket)<=1320).sort((a,b)=>bucketMinute(a.bucket)-bucketMinute(b.bucket));if(!points.length){box.innerHTML='<div class="empty">'+esc((summary&&summary.message)||'还没有 10:00-22:00 的叫号曲线。先选有线上基准的门店，或开启本机数据收集。')+'</div>';return}const w=1040,h=286,l=48,r=24,t=24,b=40,minM=600,maxM=1320,max=Math.max(10,...points.map(p=>Math.max(p.called_no_fast||0,p.called_no_typical||0,p.latest_called_no||0))),x=p=>l+(bucketMinute(p.bucket)-minM)/(maxM-minM)*(w-l-r),y=v=>h-b-(v/max)*(h-t-b);let svg='<svg viewBox="0 0 '+w+' '+h+'" preserveAspectRatio="none">';for(let i=0;i<=4;i++){const yy=t+i*(h-t-b)/4,val=Math.round(max*(4-i)/4);svg+='<line class="chart-grid" x1="'+l+'" y1="'+yy+'" x2="'+(w-r)+'" y2="'+yy+'"></line><text class="chart-label" x="4" y="'+(yy+4)+'">'+fmtN(val)+(i===0?' 号':'')+'</text>'}for(let hh=10;hh<=22;hh+=2){const xx=l+(hh*60-minM)/(maxM-minM)*(w-l-r);svg+='<line class="chart-grid" x1="'+xx+'" y1="'+t+'" x2="'+xx+'" y2="'+(h-b)+'" opacity=".55"></line><text class="chart-label" x="'+xx+'" y="'+(h-9)+'" text-anchor="middle">'+(hh<10?'0':'')+hh+':00</text>'}svg+='<line class="chart-axis" x1="'+l+'" y1="'+(h-b)+'" x2="'+(w-r)+'" y2="'+(h-b)+'"></line>';const draw=(field,color,width,dash,op)=>{const pts=points.filter(p=>p[field]>0).map(p=>x(p)+','+y(p[field])).join(' ');return pts?'<polyline points="'+pts+'" fill="none" stroke="'+color+'" stroke-width="'+width+'" '+(dash?'stroke-dasharray="'+dash+'"':'')+' stroke-linecap="round" stroke-linejoin="round" opacity="'+op+'"></polyline>':''};const fastPts=points.filter(p=>p.called_no_fast>0),slowPts=points.filter(p=>p.called_no_slow>0);if(fastPts.length&&slowPts.length){const fwd=fastPts.map(p=>x(p)+','+y(p.called_no_fast)).join(' '),back=slowPts.slice().reverse().map(p=>x(p)+','+y(p.called_no_slow)).join(' ');svg+='<polygon points="'+fwd+' '+back+'" fill="rgba(212,156,39,.16)" stroke="none"></polygon>'}svg+=draw('called_no_typical','#191817',3,'',1);const nowMin=(()=>{const dd=new Date();return dd.getHours()*60+dd.getMinutes()})();if(nowMin>=minM&&nowMin<=maxM){const nx=l+(nowMin-minM)/(maxM-minM)*(w-l-r),nhh=String(Math.floor(nowMin/60)).padStart(2,'0'),nmm=String(nowMin%60).padStart(2,'0'),na=nx>w-90?'end':'start',ntx=na==='end'?nx-4:nx+4;svg+='<line x1="'+nx+'" y1="'+t+'" x2="'+nx+'" y2="'+(h-b)+'" stroke="var(--red)" stroke-width="1.4" opacity=".85"></line><text class="chart-label" x="'+ntx+'" y="'+(t+10)+'" text-anchor="'+na+'" fill="var(--red)">现在 '+nhh+':'+nmm+'</text>'}const target=parseInt(el('qdTargetNo')?.value||'',10);if(target>0&&target<=max){const my=y(target);svg+='<line x1="'+l+'" y1="'+my+'" x2="'+(w-r)+'" y2="'+my+'" stroke="var(--red)" stroke-width="1.4" stroke-dasharray="4 4" opacity=".9"></line>'}const targetBucket=(target>0)?(advisor.target_bucket||(points.find(p=>(p.called_no_typical||0)>=target)||{}).bucket||''):'';points.forEach((p,i)=>{const cx=x(p),cy=y(p.called_no_typical||p.latest_called_no||0),sampleTip=p.source==='remote_baseline'?('样本 '+fmtN(p.sample_count||0)+' 条 / 线上聚合'):('样本 '+fmtN(p.day_count||0)+' 天 / '+fmtN(p.sample_count||0)+' 条'),tip=(p.bucket+'\n一般叫到 '+fmtN(p.called_no_typical||0)+' 号\n保守 '+fmtN(p.called_no_slow||0)+' / 偏快 '+fmtN(p.called_no_fast||0)+'\n在等 '+fmtN(p.queue_groups||p.latest_queue_groups||0)+' 桌 · 等待 '+fmtN(p.wait_minutes||p.latest_wait_minutes||0)+' 分\n'+sampleTip+'\n最近 '+shortTime(p.latest_at));svg+='<g class="chart-hot" data-tip="'+escA(tip)+'" onmousemove="dashTip(event,this)" onmouseleave="hideDashTip()"><circle cx="'+cx+'" cy="'+cy+'" r="'+((targetBucket&&p.bucket===targetBucket)?6:(i===points.length-1?5:4))+'" fill="'+((targetBucket&&p.bucket===targetBucket)||i===points.length-1?'#B81C22':'#fff')+'" stroke="'+((targetBucket&&p.bucket===targetBucket)?'#B81C22':'#191817')+'" stroke-width="'+((targetBucket&&p.bucket===targetBucket)?2.5:2)+'"></circle></g>'});svg+='</svg>';const note=(summary&&summary.message)?'<span class="mu">'+esc(summary.message)+'</span>':'';box.innerHTML=svg+'<div class="chart-legend"><span class="legend-line">一般会叫到几号</span><span class="legend-band">保守-偏快区间</span><span class="legend-now">现在</span><span class="legend-mine">我的号</span><span class="mu">只展示 10:00-22:00</span></div>'+note}
function renderDashboardCalledTable(points){const box=el('qdCalledTable');if(!box)return;points=(points||[]).slice().sort((a,b)=>bucketMinute(a.bucket)-bucketMinute(b.bucket));if(!points.length){box.innerHTML='<div class="empty">还没有叫号表。选一个有公开基准的门店，或开启本机数据收集。</div>';return}const rows=points.map(p=>{const sample=p.source==='remote_baseline'?(fmtN(p.sample_count||0)+' 条<br><span class="mu">线上聚合 · '+esc(confText(p.confidence))+'</span>'):(fmtN(p.day_count||0)+' 天<br><span class="mu">'+fmtN(p.sample_count||0)+' 条 · '+esc(confText(p.confidence))+'</span>');return'<tr><td class="num" data-label="时间">'+esc(p.bucket)+'</td><td data-label="一般叫到"><strong>'+fmtN(p.called_no_typical||0)+'号</strong><br><span class="mu">最近 '+fmtN(p.latest_called_no||0)+'号</span></td><td class="num" data-label="保守-偏快">'+fmtN(p.called_no_slow||0)+' - '+fmtN(p.called_no_fast||0)+'</td><td class="num" data-label="在等">'+fmtN(p.queue_groups||p.latest_queue_groups||0)+' 桌</td><td class="num" data-label="等待">'+fmtN(p.wait_minutes||p.latest_wait_minutes||0)+' 分</td><td data-label="样本">'+sample+'</td></tr>'}).join('');box.innerHTML='<table class="called-table"><thead><tr><th>时间</th><th>一般叫到</th><th>保守-偏快</th><th>在等</th><th>等待</th><th>样本</th></tr></thead><tbody>'+rows+'</tbody></table>'}
...
async function loadQueueBaseline(){try{const d=await safeFetch('/api/queue/baseline');window.qbStoreIDs=d.store_ids||[];window.qbUsePreferenceStores=d.use_preference_stores!==false;if(el('qbEnabled'))el('qbEnabled').checked=!!d.enabled;if(el('qbInterval'))el('qbInterval').value=d.interval_minutes||5}catch(e){}}
async function saveQueueBaseline(){const enabled=!!(el('qbEnabled')&&el('qbEnabled').checked),interval=+(el('qbInterval')?.value||5)||5;try{const d=await safeFetch('/api/queue/baseline',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:enabled,interval_minutes:interval,store_ids:window.qbStoreIDs||[],use_preference_stores:window.qbUsePreferenceStores!==false})});window.qbStoreIDs=d.store_ids||[];window.qbUsePreferenceStores=d.use_preference_stores!==false;if(el('qbEnabled'))el('qbEnabled').checked=!!d.enabled;if(el('qbInterval'))el('qbInterval').value=d.interval_minutes||5}catch(e){toast('保存基准采集设置失败')}}
async function refreshQueueView(){await loadQueueLive();loadQueueRecommend();await loadNetTicketPlan();await loadQueueAlerts();await loadQueueAlertStatus();await loadQueueBaseline();await loadQueueTrends()}
...
let qtAlerts=[];
function onQaTypeChange(){const t=(el('qaType')||{}).value;if(!el('qaWaitWrap'))return;el('qaWaitWrap').classList.toggle('hid',t!=='wait_below');el('qaTargetWrap').classList.toggle('hid',t!=='called_reach');el('qaLabelWrap').classList.toggle('hid',t!=='called_reach');el('qaTravelWrap').classList.toggle('hid',t!=='called_reach');el('qaNotifyWrap').classList.toggle('hid',t!=='called_reach');el('qaLeadWrap').classList.toggle('hid',t!=='called_reach')}
async function loadQueueAlerts(){try{const d=await safeFetch('/api/queue/alerts');qtAlerts=(d&&d.rules)||[];renderQueueAlerts();renderTicketReminderCard()}catch(e){}}
function renderQueueAlerts(){const box=el('qtAlerts');if(!box)return;if(!qtAlerts.length){box.innerHTML='<span class="mu">尚未设置提醒。先在上方选好一个关注门店，再新增提醒。</span>';return}box.innerHTML='<div class="sg">'+qtAlerts.map((r,i)=>{const point=qaRuleThreshold(r),label=r.label?('【'+r.label+'】'):'',desc=r.type==='called_reach'?(label+'多段叫号 · 我的号 '+(r.target_no||0)+' · 到/过 '+point+' 号提醒 · 提前 '+(r.lead_groups||0)+' 桌'+(r.travel_minutes?' · 路程约 '+r.travel_minutes+' 分钟':'')):('该取号了 · 预计等待 ≤ '+(r.wait_minutes||0)+' 分钟');return'<div class="sl '+(r.enabled?'av':'full')+'"><div class="ss">'+esc(r.store_name||r.store_id)+'</div><div class="mu mt8">'+esc(desc)+'</div><div class="fl g8 mt8 ai"><label class="check" style="margin:0"><input type="checkbox" '+(r.enabled?'checked':'')+' onchange="toggleQueueAlert('+i+',this.checked)">启用</label><button class="bt bt-o bt-s" onclick="removeQueueAlert('+i+')">删除</button></div></div>'}).join('')+'</div>'}
...
function addQueueAlert(){const s=currentAlertStore();if(!s){toast('请先在上方“关注门店”里选择一个门店');return}const type=(el('qaType')||{}).value||'wait_below';if(type==='called_reach'){const t=parseInt((el('qaTarget')||{}).value,10);if(!t){toast('请填写你手里的号');return}const points=alertNoList((el('qaNotifyAt')||{}).value),lead=parseInt((el('qaLead')||{}).value,10)||0,label=(el('qaLabel')?.value||'').trim(),travel=Math.max(0,parseInt(el('qaTravel')?.value||'',10)||0);if(points.length){points.forEach(n=>qtAlerts.push({store_id:s.id,store_name:s.name,label:label,type:'called_reach',target_no:t,notify_at_no:n,lead_groups:Math.max(0,t-n),travel_minutes:travel,template:'advanced',enabled:true}))}else{qtAlerts.push({store_id:s.id,store_name:s.name,label:label,type:'called_reach',target_no:t,lead_groups:lead,notify_at_no:Math.max(1,t-lead),travel_minutes:travel,template:'advanced',enabled:true})}}else{const m=parseInt((el('qaWait')||{}).value,10);if(!m){toast('请填写等待分钟阈值');return}qtAlerts.push({store_id:s.id,store_name:s.name,type:'wait_below',wait_minutes:m,enabled:true})}saveQueueAlerts()}
...
async function saveQueueAlerts(){try{const d=await safeFetch('/api/queue/alerts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({rules:qtAlerts})});qtAlerts=(d&&d.rules)||qtAlerts;renderQueueAlerts();await loadQueueAlertStatus()}catch(e){toast('保存提醒失败')}}
...
async function loadSampling(){try{const d=await(await fetch('/api/sampling')).json();spCfg=d.config||{};spState=d.state||{};spAutoStart=d.autostart||{};spQueueState=d.queue_state||{};fillSamplingForm();renderSamplingStores();renderSamplingState();uSamplingSummary();renderDashboardSamplingCard()}catch(e){el('sampleState').innerHTML='<div class="ci bad">信息收集状态加载失败</div>';renderDashboardSamplingCard()}}
...
function renderSamplingStores(){const c=el('samplingStores'),h=el('sampleStoreHint');if(!c)return;if(!stores.length){c.innerHTML='<span class="mu">本机数据收集需要凭证；只看实时排队不用。</span>';if(h)h.textContent='先获取凭证后，才能记录你常去门店的历史变化。';return}const chosen=(spCfg.store_ids||[]).map(String);c.innerHTML=stores.map(s=>'<button class="chip '+(chosen.includes(String(s.id))?'on':'')+'" data-store="'+escA(String(s.id))+'">'+esc(s.nickname||s.name||s.id)+'</button>').join('');c.querySelectorAll('.chip').forEach(b=>b.onclick=()=>{b.classList.toggle('on');renderSamplingStoreHint()});renderSamplingStoreHint()}
...
async function setBootSampling(enabled){try{const d=await(await fetch('/api/sampling/autostart',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled})})).json();if(d.error){toast(d.error);return}spAutoStart=d.autostart||{};if(cp==='sm'||cp==='qd')await loadSampling();if(cp==='qt')await loadQueueTrends();toast(enabled?'已配置开机自启动':'已取消开机自启动')}catch(e){toast('操作失败')}}
...
function renderAuthImport(d){const st=el('authImportState');if(!st)return;const miss=d.missing||[],src=(d.sources||[]).join('、')||'-';st.innerHTML='<b>'+(d.saved?'<span class="ok">已保存</span>':'<span class="bad">未保存</span>')+'</b> '+esc(d.message||'')+'<br><b>识别来源：</b>'+esc(src)+'<br>'+capLine(d.capture)+(miss.length?'<br><b>还缺：</b><span class="bad">'+esc(miss.join('、'))+'</span><br><span class="mu">请再粘贴包含缺失字段的请求；如果只有一个 Authorization，程序会先同时作为查询/预约凭证使用。</span>':'')}
async function importAuthText(){const txt=(el('authImportText')?.value||'').trim();if(!txt){toast('请先粘贴手机抓包导出的 JSON、curl 或 raw headers');return}try{const d=await safeFetch('/api/auth/import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:txt})},15000);renderAuthImport(d);if(d.saved){await loadStatus();await loadMobileUA();await loadMobileAuth();lD();toast('凭证参数已导入保存，可以点击“测试基础接口”验证。')}else{toast('已解析但字段不完整：'+((d.missing||[]).join('、')||'未知'))}}catch(e){toast('导入失败：'+String(e.message||e))}}
...
async function startMobileUACapture(){try{const d=await safeFetch('/api/mobile-ua/capture/start',{method:'POST'},12000);renderMobileUA(d);toast('已启动扫码采集。请用手机微信扫描二维码或打开局域网链接。')}catch(e){toast('启动采集失败：'+String(e.message||e))}}
async function stopMobileUACapture(){try{renderMobileUA(await safeFetch('/api/mobile-ua/capture/stop',{method:'POST'}))}catch(e){toast('停止采集失败')}}
async function saveManualMobileUA(){const ua=(el('mobileUaManual')?.value||'').trim();if(!ua){toast('请先粘贴 UA');return}try{const d=await safeFetch('/api/mobile-ua',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_agent:ua})});renderMobileUA(d.status||d);toast('已保存移动端 UA')}catch(e){toast('保存失败：'+String(e.message||e))}}
...
function discResp(r){const p=[];if(r.response_kind)p.push('resp '+r.response_kind);if(r.response_keys?.length)p.push('keys '+r.response_keys.join(', '));if(r.response_array_len)p.push('array '+r.response_array_len);if(r.response_array_item_keys?.length)p.push('item '+r.response_array_item_keys.join(', '));if(r.response_data_kind)p.push('data '+r.response_data_kind);if(r.response_data_keys?.length)p.push('data keys '+r.response_data_keys.join(', '));if(r.response_data_array_len)p.push('data array '+r.response_data_array_len);if(r.response_data_array_item_keys?.length)p.push('data item '+r.response_data_array_item_keys.join(', '));if(r.response_error_fields)p.push('error '+discMap(r.response_error_fields));return p.join(' · ')||'-'}
async function loadDiscovery(){const state=el('discState');try{const d=await safeFetch('/api/discovery');const c=d.config||{};if(el('discEnabled'))el('discEnabled').checked=!!c.enabled;if(state)state.innerHTML='<b>'+esc(c.enabled?'调试已开启':'调试未开启')+'</b><br>记录数：'+esc(d.records_count||0)+'；上限：'+esc(c.max_records||500)+'<br>文件：<code>'+esc(d.records_path||'')+'</code>';await loadDiscoveryRecords()}catch(e){if(state)state.innerHTML='<span class="bad">加载调试配置失败：'+esc(String(e.message||e))+'</span>'}}
async function saveDiscovery(){try{const enabled=!!(el('discEnabled')&&el('discEnabled').checked);const d=await safeFetch('/api/discovery',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:enabled,max_records:500})});if(d.error){toast(d.error);return}await loadDiscovery();toast(enabled?'接口发现调试已开启，请点击“获取凭证”后在小程序里点一遍相关页面。':'接口发现调试已关闭')}catch(e){toast('保存调试设置失败：'+String(e.message||e))}}
async function loadDiscoveryRecords(){const box=el('discRecords');if(!box)return;box.innerHTML='<span class="mu">读取调试记录中...</span>';try{const d=await safeFetch('/api/discovery/records?limit=80');const records=d.records||[];if(!records.length){box.innerHTML='<span class="mu">暂无调试记录。开启调试后重新获取凭证，并在小程序里点击“我的预约”等页面。</span>';return}box.innerHTML='<div class="disc-list">'+records.slice().reverse().map(r=>'<div class="sl '+((r.status||0)>=400?'full':'av')+'"><div class="fl ai jb g8 fw"><div class="ss">'+esc(discTime(r.timestamp))+' · '+esc(r.method||'-')+' '+esc(r.path||'-')+'</div><span class="pill">'+esc(r.status||'-')+'</span></div><div class="mu mt8">headers：'+esc(discJoin(r.request_header_keys))+'</div><div class="mu">query：'+esc(discJoin(r.query_keys))+'</div><div class="mu">query fields：'+esc(discMap(r.query_fields))+'</div><div class="mu">body fields：'+esc(discMap(r.request_body_fields))+'</div><div class="mu">response：'+esc(discResp(r))+'</div>'+(r.diagnosis?.length?'<div class="mu mt8"><b>诊断：</b>'+esc(r.diagnosis.join('；'))+'</div>':'')+'</div>').join('')+'</div>'}catch(e){box.innerHTML='<div class="ci bad">读取调试记录失败：'+esc(String(e.message||e))+'</div>'}}
async function clearDiscoveryRecords(){if(!await confirmDialog('清空本地接口调试记录？'))return;try{const d=await safeFetch('/api/discovery/clear',{method:'POST'});if(d.error){toast(d.error);return}await loadDiscovery();toast('已清空')}catch(e){toast('清空失败：'+String(e.message||e))}}
...
async function stopProcesses(){if(!await confirmDialog('将恢复代理、停止后台抢号/信息收集，并退出当前应用窗口。之后就可以删除 exe 或安装目录。继续？'))return;try{const r=await fetch('/api/processes/stop',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({include_self:true})}),d=await r.json();toast(d.ok?'已发送停止请求，当前应用即将退出':'部分进程未停止，请稍后再试或重启电脑')}catch(e){toast('已发送停止请求，当前应用即将退出')}}
