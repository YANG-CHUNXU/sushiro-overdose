...
<main class="shell wrap">
  ...
  <section id="p-qd" class="hid">
    <div class="cd">
      ...
      <div id="qdSamplingCard" class="curve-sampling"><div><b>本机持续采集</b><p>开启后会记录你关心门店的叫号、等位和可预约时段，曲线会越用越准；数据只留在本机，不上传。需要先完成凭证。</p></div><div class="curve-sampling-actions"><button class="bt bt-w bt-s" onclick="go('sm')">详细配置</button></div></div>
      ...
      <details class="card adv mt16">
        <summary><span class="setting-fold-title"><b>更多历史依据</b><span>10 分钟叫号表、日期类型拆分和数据说明；主图已经优先融合本机采集数据。</span></span></summary>
        <div class="mt16">
  ...
  <section id="p-se" class="hid">
    <div class="settings-grid">
      ...
      <details class="cd setting-fold">
        <summary><span class="setting-fold-title"><b>云端数据</b><span>登录 GitHub 后使用线上 Turso 基准；数据库凭证只放在 Cloudflare Worker secrets。</span></span></summary>
        <div class="setting-fold-body">
        <div id="cloudState" class="diag-detail">尚未加载</div>
        <div class="fg mt16"><label>Cloudflare Worker URL</label><input type="url" id="cloudUrl" placeholder="https://sushiro-cloud.your-name.workers.dev"></div>
        <div class="fl g8 fw mt8"><button class="bt bt-r bt-s" onclick="saveCloudAuth()">保存 URL</button><button class="bt bt-w bt-s" onclick="startCloudLogin()">用 GitHub 登录</button><button class="bt bt-w bt-s" onclick="testCloudAuth()">验证连接</button><button class="bt bt-o bt-s" onclick="logoutCloudAuth()">退出云端</button></div>
        <div class="ps mt8">这和寿司郎小程序凭证无关：云端登录只用于读取线上排队基准和未来受控上传采样；不会取号、取消号，也不会把 Turso token 写入本机。</div>
        </div>
      </details>
      ...
      <details class="cd setting-fold">
        ...
        <div class="setting-fold-body">
        <div class="fl ai jb mb16 fw g8"><div class="cd-t" style="margin-bottom:0">手机凭证捕获</div><div class="fl g8 fw"><button class="bt bt-r bt-s" onclick="openAuthWizard()">获取凭证向导</button><button class="bt bt-w bt-s" onclick="startMobileAuthCapture()">启动手机捕获</button><button class="bt bt-w bt-s" onclick="loadMobileAuth()">刷新</button><button class="bt bt-o bt-s" onclick="resetAuthOnly(true)">重置认证</button><button class="bt bt-o bt-s" onclick="stopMobileAuthCapture()">停止</button></div></div>
        <div class="ps">用于 Windows 端：手机真实微信产生寿司郎凭证流量，电脑只捕获凭证参数。寿司郎凭证会过期，也可能被手机端登录顶掉；遇到 E010/error.server 时请先重置认证再重新获取。不会修改 Windows 系统代理；捕获完成后请关闭手机 Wi-Fi 代理。</div>
...
<script>
let cp='da',es={status:'idle'},hc=0,as=[],sd='',pr={},pf='',cE=null,stores=[],selStores=[],calErrs=[],arTimer=null,lastDiag=null,spCfg={},spState={status:'idle'},spAutoStart={},spQueueState={},qdSelected=[],qtSelected=[],qtTrendStores=[],qaStatus={},ah={},nfc=true,cloudAuth={},qpStore='',qwStore='';
...
function uAuth(){const pill=el('authPill'),banner=el('authBanner'),st=(ah&&ah.status)||'unknown',reason=(ah&&ah.reason)?String(ah.reason):'';if(pill){if(!hc){pill.className='authpill';pill.textContent='只读模式'}else if(st==='stale'){pill.className='authpill stale';pill.textContent='凭证可能过期'}else{pill.className='authpill ok';pill.textContent='已凭证'}pill.classList.remove('hid')}if(banner){if(hc&&st==='stale'){banner.classList.remove('hid');banner.innerHTML='<b>凭证可能已过期</b>'+(reason?esc(reason)+'。':'')+'寿司郎凭证会过期，也可能被手机端登录顶掉。请先重置认证，再重新获取凭证。<button class="bt bt-r bt-s" onclick="event.stopPropagation();resetAuthAndStart()">重置并重新认证</button>'}else{banner.classList.add('hid');banner.innerHTML=''}}}
function authPillClick(){if(hc&&(ah&&ah.status==='stale'))resetAuthAndStart();else if(!hc)startAuth();else go('re')}
async function init(){consumeCloudAuthResult();await loadStatus();await lP();checkUpdate();sse();const h=location.hash.slice(1);if(h&&PAGE_GROUP[h]&&h!=='da')go(h);else maybeShowIntro();}
...
async function resetAuthAndStart(){if(!await resetAuthOnly(true))return;if(pf==='windows')openAuthWizard();else sC()}
...
function startAuth(){if(hc&&(ah&&ah.status==='stale')){resetAuthAndStart();return}if(pf==='windows'){openAuthWizard()}else{sC()}}
...
async function loadQueueAdvisorCard(){const ans=el('qdAnswer');if(!ans)return;const store=qdSelected[0]||'';if(!store){ans.innerHTML='<div class="ci">选一家门店，并填上你手里的号，这里直接给你「几点叫到、几点出发」。</div>';return}const target=parseInt(el('qdTargetNo')?.value||'',10)||0,travel=Math.max(0,parseInt(el('qdrTravel')?.value||'',10)||0);ans.innerHTML='<div class="ci">正在读取实时排队压力…</div>';try{const qs='store='+encodeURIComponent(store)+(target>0?'&target_no='+target:'')+(travel>0?'&travel_minutes='+travel:'');const adv=await safeFetch('/api/queue/advisor?'+qs,null,15000);renderQueueAnswer(adv,target)}catch(e){ans.innerHTML=loadErrBoxHTML(e,'loadQueueAdvisorCard()','排队压力')}}
...
function renderDashboardCalledCurve(points,summary,advisor){
 ...
 if(!points.length){box.innerHTML='<div class="empty">'+esc((summary&&summary.message)||'还没有 10:00-22:00 的叫号曲线。先选有线上基准的门店，或开启本机数据收集。')+'</div>';return}
...
function renderCloudAuth(d){cloudAuth=d||{};const st=el('cloudState');if(el('cloudUrl')&&!el('cloudUrl').value)el('cloudUrl').value=cloudAuth.base_url||'';if(!st)return;const cfg=!!cloudAuth.configured,conn=!!cloudAuth.connected,user=cloudAuth.user_login||'',msg=cloudAuth.last_error?('<br><span class="bad">'+esc(cloudAuth.last_error)+'</span>'):'',who=conn?(esc(user||'GitHub')+(cloudAuth.expires_at?(' · 到期 '+esc(shortTime(cloudAuth.expires_at))):'')):(cfg?'未登录 GitHub':'未配置 Worker');st.innerHTML=chip('Worker',cfg?(cloudAuth.base_url||'已配置'):'未配置',cfg?'ok':'warn')+chip('云端会话',who,conn?'ok':cfg?'warn':'warn')+chip('Turso 凭证','只在 Worker secrets',conn?'ok':'warn')+msg+'<div class="mu mt8">'+esc(cloudAuth.provider_message||'登录只用于云端基准，不影响本机抢号凭证。')+'</div>'}
...
async function saveCloudAuth(){const base=(el('cloudUrl')?.value||'').trim();try{const d=await safeFetch('/api/cloud/auth',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({base_url:base})});renderCloudAuth(d);toast(base?'已保存云端 Worker URL':'已清空云端 Worker URL');return true}catch(e){toast('保存云端设置失败：'+String(e.message||e));return false}}
async function startCloudLogin(){const base=(el('cloudUrl')?.value||'').trim();if(base&&base!==(cloudAuth.base_url||'')){if(!await saveCloudAuth())return}if(!(cloudAuth.configured||base)){toast('请先填写 Cloudflare Worker URL');return}location.href='/api/cloud/auth/start'}
