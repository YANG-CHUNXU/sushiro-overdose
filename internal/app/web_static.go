...
<main class="shell wrap">
      ...
      <div>
        <div class="hero">
          ...
          <div class="actions">
            ...
            <button class="bt bt-w" id="bc" onclick="startAuth()">获取认证</button>
        ...
        <div class="task-grid">
          <button class="task-card" onclick="go('qt')" type="button">
            <span class="tag read">只读 · 不用认证</span>
            <h3>现在想去吃</h3>
      ...
      <aside class="side">
        ...
        <details class="card adv">
          ...
          <div class="safety-grid" style="margin-top:12px">
            <div class="safety-card"><b><span class="safe-tag safe-read">只读</span>不会影响账号</b><p>看门店、看排队、看叫号预测，不需要你的账号认证。</p></div>
            <div class="safety-card"><b><span class="safe-tag safe-auth">需要认证</span>只读取状态</b><p>读取你的预约和排队信息，用于展示、提醒和恢复本地状态。</p></div>
        ...
        <div class="card">
          <h2>第一次使用</h2>
          <div class="track">
            <div class="step" id="step-capture" style="cursor:pointer" onclick="startAuth()"><div class="mark">1</div><div><b>需要自动抢时再获取认证</b><span>只看排队不用认证；抢预约、取号、读取我的单据才需要。</span></div></div>
  ...
  <section id="p-ca" class="hid">
    <div class="cd">
      <div class="fl ai jb mb16 fw g8">
        <div><h2 class="ph">预约未来</h2><p class="ph-sub">查询未来日期是否有可预约时段；需要自动创建预约时才会使用你的认证。</p></div>
    ...
    <div class="cd">
      <div class="dash-head">
        <div>
          ...
          <p class="dash-copy"><span class="tag read">只读 · 不用认证</span> 选门店、输入号码后，系统会直接估算大概几点叫到和建议到店时间；图表只是解释依据。</p>
        </div>
      ...
      <div id="qdAdvisor" class="advisor-panel"><div class="ci">先选门店，再输入你手里的号；这里会显示大概几点叫到和几点前到店。</div></div>
      ...
      <div id="qdSamplingCard" class="curve-sampling"><div><b>本机持续采集</b><p>开启后会记录你关心门店的叫号、等位和可预约时段，曲线会越用越准；数据只留在本机，不会上传。需要先完成认证。</p></div><div class="curve-sampling-actions"><button class="bt bt-w bt-s" onclick="go('sm')">详细配置</button></div></div>
      <div class="mt16">
  ...
  <section id="p-qt" class="hid">
    <div class="cd">
      <div class="page-lead"><div><h2 class="ph">现在去吃</h2><p class="ph-sub"><span class="tag read">只读 · 不用认证</span> 先看门店是否营业、前面还有几桌、大概等多久；只有点击远程取号或启用自动取号时才会执行操作。</p></div><div class="fl g8 fw"><button class="bt bt-w bt-s" onclick="refreshQueueView()">刷新</button></div></div>
      ...
      <div class="qbox mt16">
        <div class="fl ai jb fw g8"><label style="margin:0">全国门店基准采集 <span class="tag read">只读</span></label><span class="mu">周期快照全部门店等位/状态写本地作为基准；走公开接口，无需认证。</span></div>
  ...
  <section id="p-sm" class="hid">
    <div class="cd">
      <div class="page-lead"><div><h2 class="ph">本机数据收集</h2><p class="ph-sub"><span class="tag auth">需要认证</span> 只记录你关心门店的预约和排队变化，用来提升“几点叫到”的判断；不会自动取消或创建预约。</p></div><div class="fl g8 fw"><button class="bt bt-w bt-s" onclick="runSampleOnce()">立即收集一次</button><button class="bt bt-r bt-s" onclick="startSampling()">开始信息收集</button></div></div>
  ...
  <section id="p-re" class="hid">
    <div class="cd"><h2 class="ph">我的预约 / 排队号</h2><p class="ph-sub mb16"><span class="tag auth">需要认证</span> 这里用于查看官方预约和排队号；取消按钮是危险操作，会单独确认。</p><div id="rc"><div class="empty">加载中</div></div></div>
  </section>

  <section id="p-se" class="hid">
    <div class="settings-grid">
      <div class="section-banner"><b>常用设置</b><span>普通用户只需要设置门店、人数、时间偏好和通知。认证、诊断、接口调试都放在下方高级工具区。</span></div>
      <div class="cd">
      ...
      <details class="cd setting-fold">
        <summary><span class="setting-fold-title"><b>手机认证捕获</b><span>手机代理抓取真实微信认证参数；只在需要抢预约、读我的单据时使用。</span></span></summary>
        <div class="setting-fold-body">
      ...
      <details class="cd setting-fold">
        <summary><span class="setting-fold-title"><b>手动导入认证</b><span>手机抓包后粘贴 JSON、curl 或原始请求头；适合路由器隔离时使用。</span></span></summary>
        <div class="setting-fold-body">
        <div class="fl ai jb mb16 fw g8"><div class="cd-t" style="margin-bottom:0">手动导入认证</div><div class="fl g8 fw"><button class="bt bt-r bt-s" onclick="importAuthText()">解析并保存</button><button class="bt bt-w bt-s" onclick="el('authImportText').value=''">清空</button></div></div>
        <div class="ps">网络隔离导致手机连不上电脑代理时使用：在手机本机抓包工具里复制寿司郎请求的 JSON、curl 或 raw headers，粘贴到这里。建议至少包含一次门店/查询请求和一次预约或排队请求。</div>
        <div class="fg mt8"><label>认证内容</label><textarea id="authImportText" placeholder="支持 JSON / curl / raw headers。字段包括 X-App-Code、Authorization、User-Agent、Referer、wechatId、phoneNumber、storeId。"></textarea></div>
...
<script>
let cp='da',es={status:'idle'},hc=0,as=[],sd='',pr={},pf='',cE=null,stores=[],selStores=[],calErrs=[],arTimer=null,lastDiag=null,spCfg={},spState={status:'idle'},spAutoStart={},spQueueState={},qdSelected=[],qtSelected=[],qtTrendStores=[],qaStatus={},ah={};
...
function uD(){
  ...
  if(es.status==='capturing'){
    badge.textContent='正在获取认证';title.textContent='按引导操作一次小程序';copy.textContent='只需要点进寿司郎小程序产生一次请求，不要提交预约，也不要取消任何订单。抓到字段后页面会自动更新。';
    ...
    badge.textContent='第一次使用';title.textContent='先不用认证，直接看排队';copy.textContent='只看全国门店、实时排队和叫号预测不需要账号认证。只有抢未来预约、远程取号、读取我的单据时才需要获取认证。';
    b.textContent='现在去吃：看排队';b.className='bt bt-r bt-l';b.onclick=()=>go('qt');
    bc.textContent=pf==='windows'?'我要抢预约，手机扫码认证':'我要抢预约，获取认证';
    bc.onclick=startAuth;
    nc.classList.remove('hid');nc.textContent='建议先从“现在去吃”或“我有号码”开始；确认要自动操作时再做认证。';
  }else{
    const hasStores=(pr.selected_stores||[]).length>0;
    if(!hasStores){
      badge.textContent='认证已保存';title.textContent='下一步：选择门店和偏好';copy.textContent='抢未来预约前，需要先选门店、人数、桌型和时间偏好。只看排队仍然可以直接使用。';
      b.textContent='设置门店和偏好';b.className='bt bt-y bt-l';b.onclick=()=>go('se');
      bc.textContent='先看实时排队';bc.onclick=()=>go('qt');
    }else{
      badge.textContent='准备就绪';title.textContent='可以查日历，也可以自动蹲号';copy.textContent='已保存认证和门店偏好。建议先查看预约日历；如果目标明确，再启动自动蹲号或自动抢预约。';
      b.textContent='查看预约日历';b.className='bt bt-r bt-l';b.onclick=()=>go('ca');
      bc.textContent='开始自动抢预约';
      bc.className='bt bt-o';
      bc.onclick=sB;
      brst.textContent='重新获取认证';
...
async function rST(){if(!await confirmDialog('重置抓包状态？会断开当前抓包代理并清理残留，之后可点「获取认证」手动重新连接。'))return;try{const d=await safeFetch('/api/engine/reset',{method:'POST'});if(d.error){toast(d.error);return}await loadStatus();toast('已重置抓包状态，点「获取认证」可重新连接')}catch(e){toast('重置失败：'+String(e.message||e))}}
...
function openFirstUseWizard(){let ov=el('firstUse');if(!ov){ov=document.createElement('div');ov.id='firstUse';ov.className='ov';document.body.appendChild(ov)}ov.innerHTML='<div class="ovc" style="width:min(820px,96vw)"><div class="fl ai jb mb16"><div><b>你现在想做什么？</b><div class="mu mt8">先选场景；只读功能会直接进入，需要认证或会执行操作的功能会先提示。</div></div><button class="bt bt-w bt-s" onclick="closeFirstUseWizard()">关闭</button></div><div class="task-grid"><button class="task-card" type="button" onclick="firstUseGo(\'qt\',false)"><span class="tag read">只读</span><h3>现在想去吃</h3><p>看门店营业、等位和实时叫号，不需要认证。</p><div class="task-foot"><span class="mu">直接进入</span><span class="task-arrow">›</span></div></button><button class="task-card" type="button" onclick="firstUseGo(\'qd\',false)"><span class="tag read">只读</span><h3>我已经拿到号</h3><p>输入号码，判断大概几点叫到、几点前到店。</p><div class="task-foot"><span class="mu">直接进入</span><span class="task-arrow">›</span></div></button><button class="task-card" type="button" onclick="firstUseGo(\'ca\',true)"><span class="tag auth">需要认证</span><h3>想约未来某天</h3><p>查预约日历，准备按偏好自动抢预约。</p><div class="task-foot"><span class="mu">没有认证会先引导认证</span><span class="task-arrow">›</span></div></button><button class="task-card" type="button" onclick="firstUseGo(\'sn\',true)"><span class="tag action">会执行操作</span><h3>自动蹲号</h3><p>设置目标时段，到窗口自动尝试创建预约。</p><div class="task-foot"><span class="mu">启动前还会再次确认</span><span class="task-arrow">›</span></div></button></div></div>';ov.classList.remove('hid');ov.style.display='flex'}
function closeFirstUseWizard(){const ov=el('firstUse');if(ov){ov.classList.add('hid');ov.style.display='none'}}
async function firstUseGo(page,needsAuth){closeFirstUseWizard();if(needsAuth&&!hc){if(await confirmDialog({title:'需要先获取认证',body:'这个功能需要先获取认证。\\n只看排队和叫号预测不用认证；抢预约或自动蹲号才需要。\\n现在去获取认证？',ok:'去获取认证',cancel:'先看看'}))startAuth();else go(page);return}go(page)}
...
async function refreshQueueView(){await loadQueueLive();await loadNetTicketPlan();await loadQueueAlerts();await loadQueueAlertStatus();await loadQueueBaseline();await loadQueueTrends()}
let qtAlerts=[];
...
function renderSamplingStores(){const c=el('samplingStores'),h=el('sampleStoreHint');if(!c)return;if(!stores.length){c.innerHTML='<span class="mu">本机数据收集需要认证；只看实时排队不用。</span>';if(h)h.textContent='先获取认证后，才能记录你常去门店的历史变化。';return}const chosen=(spCfg.store_ids||[]).map(String);c.innerHTML=stores.map(s=>'<button class="chip '+(chosen.includes(String(s.id))?'on':'')+'" data-store="'+escA(String(s.id))+'">'+esc(s.nickname||s.name||s.id)+'</button>').join('');c.querySelectorAll('.chip').forEach(b=>b.onclick=()=>{b.classList.toggle('on');renderSamplingStoreHint()});renderSamplingStoreHint()}
...
async function loadMobileAuth(){try{renderMobileAuth(await safeFetch('/api/mobile-auth'))}catch(e){const st=el('mobileAuthState');if(st)st.innerHTML='<span class="bad">加载手机认证状态失败：'+esc(String(e.message||e))+'</span>'}}
async function startMobileAuthCapture(){try{const d=await safeFetch('/api/mobile-auth/start',{method:'POST'},12000);renderMobileAuth(d);toast('已启动手机认证捕获。请用手机微信扫码打开引导页，按步骤安装 CA、设置 Wi-Fi 代理，再打开寿司郎小程序。')}catch(e){toast('启动失败：'+String(e.message||e))}}
async function stopMobileAuthCapture(){try{renderMobileAuth(await safeFetch('/api/mobile-auth/stop',{method:'POST'}));toast('已停止手机认证捕获，请确认手机 Wi-Fi 代理已关闭。')}catch(e){toast('停止失败：'+String(e.message||e))}}
function renderAuthImport(d){const st=el('authImportState');if(!st)return;const miss=d.missing||[],src=(d.sources||[]).join('、')||'-';st.innerHTML='<b>'+(d.saved?'<span class="ok">已保存</span>':'<span class="bad">未保存</span>')+'</b> '+esc(d.message||'')+'<br><b>识别来源：</b>'+esc(src)+'<br>'+capLine(d.capture)+(miss.length?'<br><b>还缺：</b><span class="bad">'+esc(miss.join('、'))+'</span><br><span class="mu">请再粘贴包含缺失字段的请求；如果只有一个 Authorization，程序会先同时作为查询/预约认证使用。</span>':'')}
async function importAuthText(){const txt=(el('authImportText')?.value||'').trim();if(!txt){toast('请先粘贴手机抓包导出的 JSON、curl 或 raw headers');return}try{const d=await safeFetch('/api/auth/import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:txt})},15000);renderAuthImport(d);if(d.saved){await loadStatus();await loadMobileUA();await loadMobileAuth();lD();toast('认证参数已导入保存，可以点击“测试基础接口”验证。')}else{toast('已解析但字段不完整：'+((d.missing||[]).join('、')||'未知'))}}catch(e){toast('导入失败：'+String(e.message||e))}}
...
async function saveDiscovery(){try{const enabled=!!(el('discEnabled')&&el('discEnabled').checked);const d=await safeFetch('/api/discovery',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:enabled,max_records:500})});if(d.error){toast(d.error);return}await loadDiscovery();toast(enabled?'接口发现调试已开启，请点击“获取认证”后在小程序里点一遍相关页面。':'接口发现调试已关闭')}catch(e){toast('保存调试设置失败：'+String(e.message||e))}}
async function loadDiscoveryRecords(){const box=el('discRecords');if(!box)return;box.innerHTML='<span class="mu">读取调试记录中...</span>';try{const d=await safeFetch('/api/discovery/records?limit=80');const records=d.records||[];if(!records.length){box.innerHTML='<span class="mu">暂无调试记录。开启调试后重新获取认证，并在小程序里点击“我的预约”等页面。</span>';return}box.innerHTML='<div class="disc-list">'+records.slice().reverse().map(r=>'<div class="sl '+((r.status||0)>=400?'full':'av')+'"><div class="fl ai jb g8 fw"><div class="ss">'+esc(discTime(r.timestamp))+' · '+esc(r.method||'-')+' '+esc(r.path||'-')+'</div><span class="pill">'+esc(r.status||'-')+'</span></div><div class="mu mt8">headers：'+esc(discJoin(r.request_header_keys))+'</div><div class="mu">query：'+esc(discJoin(r.query_keys))+'</div><div class="mu">query fields：'+esc(discMap(r.query_fields))+'</div><div class="mu">body fields：'+esc(discMap(r.request_body_fields))+'</div><div class="mu">response：'+esc(discResp(r))+'</div>'+(r.diagnosis?.length?'<div class="mu mt8"><b>诊断：</b>'+esc(r.diagnosis.join('；'))+'</div>':'')+'</div>').join('')+'</div>'}catch(e){box.innerHTML='<div class="ci bad">读取调试记录失败：'+esc(String(e.message||e))+'</div>'}}
