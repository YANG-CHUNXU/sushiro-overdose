      ...
      <div>
        ...
        <div id="homeDecisionPanel" class="home-decision mt16">
          ...
          <div class="home-decision-grid">
            ...
            <button class="home-decision-card auth" onclick="go('ca')" type="button">
              <span>想约未来某天</span>
              <b>查未来预约</b>
...
function renderPressureChart(box,curve,adv,target){
 ...
 if(trendPts.length>1)notes.push('历史排队趋势：绿色虚线是 '+queueTrendSourceLabel(trend[0].source)+' 的 '+(trend.length)+' 个时间窗按 total_queue_groups 独立归一化（与叫号轴量纲不同，仅看高低走势）。');
 const note=notes.length?'<div class="mu mt8">'+esc(notes.join(' '))+'</div>':'';
 box.innerHTML=svg+'<div class="chart-legend"><span class="legend-line">今日叫号</span><span class="legend-history">历史推算叫号</span>'+(trendPts.length>1?'<span class="legend-turso-trend">'+(qdDashboardData.scope&&qdDashboardData.scope.mode==='all'?'全国 ':'')+queueTrendSourceLabel(trend[0]&&trend[0].source)+'排队趋势</span>':'')+'<span class="legend-pressure">排队压力（灰带越满压力越大）</span><span class="legend-now">现在</span><span class="legend-mine">我的当天排队号</span><span class="mu">只展示 10:00-22:00</span></div>'+note
}
