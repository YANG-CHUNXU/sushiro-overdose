...
function renderNetTicketRoutineStatus(r){const box=el('nrStatus');if(!box)return;r=r||{};if(!r.enabled){box.innerHTML='<span class="mu">未开启 Routine。开启后，本工具会每天按目标就餐时间倒推取号时间；样本不足时不会自动取号。</span>';return}const store=esc(r.store_name||storeDisplayName(r.store_id)||r.store_id||''),meal=r.target_meal_time?netTimeDisp(r.target_meal_time):'-',pickup=r.planned_pickup_time||'',range=r.recommend_pickup_range?(r.recommend_pickup_range.early+'-'+r.recommend_pickup_range.late):'',wait=r.wait_minutes_range?('预计等 '+r.wait_minutes_range.low+'-'+r.wait_minutes_range.high+' 分钟'):'等待样本不足',risk=r.risk==='high'?'风险偏高':r.risk==='medium'?'风险中等':r.risk==='low'?'风险较低':'风险待确认';let head='',detail='';switch(r.status){case'armed':head='已开启：今天预计 '+pickup+' 自动取号';detail=store+' · 目标 '+meal+' 吃 · '+wait+(range?' · 建议窗口 '+range:'')+' · '+risk;break;case'waiting_data':head='已开启：等待历史样本';detail=r.last_error||'这家店样本不足，暂不自动取号。去“预测准确度”开启本机采集后会自动补齐。';break;case'blocked':head='已开启：被手动计划暂停';detail=r.last_error||'已有手动自动取号计划，Routine 不会覆盖它。';break;case'missed':head='今天已错过推算取号时间';detail=r.last_error||'Routine 明天会重新规划。';break;case'done':head='今天已处理过自动取号';detail=r.last_error||'取号结果请看“我的单据”。';break;case'error':head='Routine 保存失败';detail=r.last_error||'未知错误';break;default:head='已开启：等待下一次规划';detail='目标 '+meal+' 吃，后台会按历史等待倒推取号时间。'}box.innerHTML='<b>'+esc(head)+'</b><div class="mu mt8">'+esc(detail)+(r.basis?'<br>'+esc(r.basis):'')+'</div><div class="fl g8 fw mt8"><button class="bt bt-w bt-s" onclick="go(&quot;sm&quot;)">提升预测准确度</button><button class="bt bt-w bt-s" onclick="refreshNetTicketRoutineNow()">重新试算今天</button></div>'}
