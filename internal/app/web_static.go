...
async function verifyAuthTicket(){
  ...
  try{const d=await safeFetch('/api/auth/verify',{method:'POST'},20000);
    ...
    if(box)box.innerHTML='<div class="ci '+cls+'">'+(d.valid?'✓ ':d.ok?'✕ ':'! ')+esc(d.message||'')+'</div>'+(d.detail?'<div class="mu mt8"><code style="word-break:break-all">'+esc(d.detail)+'</code></div>':'');
    await loadStatus();
    toast(d.valid?'凭证有效':d.ok?'凭证已失效，请重新获取':'未能判定，请稍后重试',d.valid?'ok':d.ok?'warn':'');
