    ...
    <div class="cd">
      <div class="dash-head">
        ...
        <div class="dash-controls">
          ...
          <label class="dash-target">我的号 <input id="qdTargetNo" type="number" min="1" placeholder="如 893" onchange="loadQueueDashboard()" onkeydown="if(event.key==='Enter')loadQueueDashboard()"></label>
...
async function lQD(){await ensureStores();renderDashboardStores();await loadSampling();await loadQueueDashboard()}
...
function applyDashboardStores(ids){qdSelected=(ids||[]).slice(0,1).map(String);if(qdSelected.length&&el('qdScope'))el('qdScope').value='local';renderDashboardStores();loadQueueDashboard()}
function renderDashboardStores(){const c=el('qdStores');if(!c)return;if(!qdSelected.length){c.innerHTML='<span class="mu">未指定门店：主图会自动选择本机叫号样本最多、最新的门店。</span>';return}c.innerHTML=qdSelected.map(id=>'<button class="chip on" data-store="'+escA(String(id))+'">'+esc(storeDisplayName(id))+' ✕</button>').join('')+'<button class="chip" onclick="qdSelected=[];renderDashboardStores();loadQueueDashboard()">清空</button>';c.querySelectorAll('.chip.on').forEach(b=>b.onclick=()=>{const id=b.dataset.store;qdSelected=qdSelected.filter(x=>x!==id);renderDashboardStores();loadQueueDashboard()})}
...
let qtAlerts=[];
function onQaTypeChange(){const t=(el('qaType')||{}).value;if(!el('qaWaitWrap'))return;el('qaWaitWrap').classList.toggle('hid',t!=='wait_below');el('qaTargetWrap').classList.toggle('hid',t!=='called_reach');el('qaNotifyWrap').classList.toggle('hid',t!=='called_reach');el('qaLeadWrap').classList.toggle('hid',t!=='called_reach')}
async function loadQueueAlerts(){try{const d=await safeFetch('/api/queue/alerts');qtAlerts=(d&&d.rules)||[];renderQueueAlerts()}catch(e){}}
function renderQueueAlerts(){const box=el('qtAlerts');if(!box)return;if(!qtAlerts.length){box.innerHTML='<span class="mu">尚未设置提醒。先在上方选好一个关注门店，再新增提醒。</span>';return}box.innerHTML='<div class="sg">'+qtAlerts.map((r,i)=>{const point=(r.notify_at_no||((r.target_no||0)-(r.lead_groups||0))||0),desc=r.type==='called_reach'?('多段叫号 · 我的号 '+(r.target_no||0)+' · 到/过 '+point+' 号提醒 · 提前 '+(r.lead_groups||0)+' 桌'):('该取号了 · 预计等待 ≤ '+(r.wait_minutes||0)+' 分钟');return'<div class="sl '+(r.enabled?'av':'full')+'"><div class="ss">'+esc(r.store_name||r.store_id)+'</div><div class="mu mt8">'+esc(desc)+'</div><div class="fl g8 mt8 ai"><label class="check" style="margin:0"><input type="checkbox" '+(r.enabled?'checked':'')+' onchange="toggleQueueAlert('+i+',this.checked)">启用</label><button class="bt bt-o bt-s" onclick="removeQueueAlert('+i+')">删除</button></div></div>'}).join('')+'</div>'}
...
async function saveQueueAlerts(){try{const d=await safeFetch('/api/queue/alerts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({rules:qtAlerts})});qtAlerts=(d&&d.rules)||qtAlerts;renderQueueAlerts()}catch(e){toast('保存提醒失败')}}
