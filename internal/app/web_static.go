...
@media(max-width:900px){
  .grid,.settings-grid,.sn-row,.dash-split,.dash-head{grid-template-columns:1fr}
...
function dashboardParams(){const p=new URLSearchParams();p.set('scope',el('qdScope')?.value||'all');p.set('date_type',el('qdType')?.value||'all');p.set('window','12');p.set('bucket',el('qdBucket')?.value||'10');if(qdSelected.length)p.set('stores',qdSelected.slice(0,1).join(','));return p}
