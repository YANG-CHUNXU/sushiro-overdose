...
<script>
let cp='da',es={status:'idle'},hc=0,as=[],sd='',pr={},pf='',cE=null,stores=[],selStores=[],calErrs=[],arTimer=null,lastDiag=null,spCfg={},spState={status:'idle'},spAutoStart={},spQueueState={},qdSelected=[],qdDashboardData={},qtSelected=[],qtTrendStores=[],qaStatus={},ah={},nfc=true,notifyChannels=[],cloudAuth={},cloudVerifyOnLoad=false;
...
async function lQD(){await ensureStores();if(!qdSelected.length){const saved=recallStores('sushiro_qd_store').slice(0,1);if(saved.length)qdSelected=saved}renderDashboardStores();fillNetTicketStores();loadNetTicketRoutine();await loadSampling();await loadQueueAlerts();await loadQueueAlertStatus();await loadQueueDashboard()}
...
async function loadQueueAdvisorCard(){const ans=el('qdAnswer'),pc=el('qdPressChart');if(!ans)return;const store=qdSelected[0]||'';if(!store){qdDashboardData={};ans.innerHTML='<div class="ci">选一家门店，并填上你手里的号，这里直接给你「几点叫到、几点出发」。</div>';if(pc)pc.innerHTML='<div class="empty">选门店后，这里把今天的叫号进度、排队压力和你的号画在同一张图上。</div>';return}const target=parseInt(el('qdTargetNo')?.value||'',10)||0,travel=Math.max(0,parseInt(el('qdrTravel')?.value||'',10)||0);ans.innerHTML='<div class="ci">正在读取实时排队压力…</div>';let adv=null;try{const qs='store='+encodeURIComponent(store)+(target>0?'&target_no='+target:'')+(travel>0?'&travel_minutes='+travel:'');adv=await safeFetch('/api/queue/advisor?'+qs,null,15000);renderQueueAnswer(adv,target)}catch(e){ans.innerHTML=loadErrBoxHTML(e,'loadQueueAdvisorCard()','排队压力')}
...
function renderPressureChart(box,curve,adv,target){
 ...
 const calledMax=Math.max(10,...points.map(p=>p.called_no||0),...hist.flatMap(p=>[p.called_no_slow||0,p.called_no_typical||0,p.called_no_fast||0]));
 const w=1040,h=286,l=48,r=44,t=24,b=40,minM=600,maxM=1320,maxCalled=calledMax,x=m=>l+(Math.min(maxM,Math.max(minM,m))-minM)/(maxM-minM)*(w-l-r),yCall=v=>h-b-(v/maxCalled)*(h-t-b),yPress=s=>h-b-(Math.min(100,Math.max(0,s))/100)*(h-t-b);
 ...
 if(histPts.length>1)svg+='<polyline points="'+histPts.join(' ')+'" fill="none" stroke="var(--blue)" stroke-width="2.4" stroke-dasharray="7 5" stroke-linejoin="round" stroke-linecap="round"></polyline>';
 ...
 box.innerHTML=svg+'<div class="chart-legend"><span class="legend-line">今日叫号</span><span class="legend-history">历史推算叫号</span><span class="legend-pressure">排队压力</span><span class="legend-now">现在</span><span class="legend-mine">我的号</span><span class="mu">只展示 10:00-22:00</span></div>'+note
}
...
function renderQueueDashboard(d){renderDashboardAdvisor(d.advisor||{})}
...
async function takeTicket(id){if(!await confirmDialog('现在远程取号？\\n这会向寿司郎提交取号请求，不是只读查看。\\n取号后请尽快到店，过号会作废。'))return;try{const d=await(await fetch('/api/queue/ticket',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({store:String(id)})})).json();if(d.error){toast('取号失败：'+d.error);return}const t=d.ticket||{};toast('已取号 '+(t.number||'(详见我的单据)')+'，请到“我的单据”查看');loadActiveTickets(true)}catch(e){toast('取号失败')}}
