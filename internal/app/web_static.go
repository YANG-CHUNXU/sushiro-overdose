...
<main class="shell wrap">
    ...
    <div class="grid">
      <div>
        ...
        <div class="summary">
          ...
          <div class="mini"><span>预测准确度</span><strong id="sumSampling">未启动</strong><p id="sumSamplingSub">用于到店预测</p></div>
        </div>
      </div>
      <aside class="side">
        ...
        <div class="card">
          ...
          <div class="track">
            ...
            <div class="step" id="step-sampling" style="cursor:pointer" onclick="go('sm')"><div class="mark">3</div><div><b>提升预测准确度</b><span>本机记录常用门店叫号变化，让到店判断更准。</span></div></div>
    ...
    <div class="cd">
      <div class="dash-head">
        <div>
          ...
          <p class="dash-copy"><span class="tag read">只读 · 不用凭证</span> 用同门店同日型的历史等待估算就餐时间，并用今天的实时排队压力修正风险；只是参考，不会替你取号。</p>
        </div>
        <div class="dash-controls">
    ...
    <div class="cd">
      <div class="dash-head">
        <div>
          ...
          <p class="dash-copy"><span class="tag read">只读 · 不用凭证</span> 按历史等待倒推建议取号时间，并提示最晚别拖过几点；今天实时压力只用于修正风险等级。</p>
        </div>
        <div class="dash-controls">
  ...
  <section id="p-qt" class="hid">
    <div class="cd">
      ...
      <div class="qbox mt16">
        <div class="fl ai jb fw g8"><label style="margin:0">每日到店 Routine <span class="tag action">会执行操作</span></label><span class="mu">每天按你想吃饭的时间倒推取号时间，到点自动远程取号。</span></div>
        <div class="fl g8 fw mt8">
          ...
          <div class="fg"><label>路程(分钟)</label><input type="number" id="nrTravel" value="0" min="0" style="width:90px"></div>
          <div class="fg"><label>安全提前(分钟)</label><input type="number" id="nrSafety" value="10" min="0" style="width:100px"></div>
          <div class="fg" style="align-self:flex-end"><button class="bt bt-r bt-s" onclick="saveNetTicketRoutine(true)">启用 Routine</button></div>
          <div class="fg" style="align-self:flex-end"><button class="bt bt-o bt-s" onclick="saveNetTicketRoutine(false)">关闭 Routine</button></div>
        </div>
        <div id="nrStatus" class="pick-out mt8"><span class="mu">选择门店和目标就餐时间后，会用历史等待数据倒推今天取号时间；样本不足时不会自动猜测。</span></div>
      </div>
      ...
      <div class="qbox mt16">
        <div class="fl ai jb fw g8"><label style="margin:0">多段到店提醒</label><span class="mu">已经移到“我有号码”，按手里的号生成临时提醒点。</span></div>
        <div class="fl g8 fw mt8"><button class="bt bt-w bt-s" onclick="go('qd')">去设置多段提醒</button></div>
      </div>
      <div class="qbox mt16 debug-only">
      ...
      <div class="mt16 debug-only" id="qtAlertCard">
  ...
  <section id="p-sm" class="hid">
    <div class="cd">
      <div class="page-lead"><div><h2 class="ph">预测准确度</h2><p class="ph-sub"><span class="tag auth">需要寿司郎认证</span> 只记录你关心门店的预约、排队和叫号变化，用来提升“几点叫到、几点出发”的判断；不会自动取消或创建预约。</p></div><div class="fl g8 fw"><button class="bt bt-w bt-s" onclick="runSampleOnce()">收集一次</button><button class="bt bt-r bt-s" onclick="startSampling()">开启持续采集</button></div></div>
  ...
  <section id="p-se" class="hid">
    <div class="settings-grid">
      ...
      <details class="cd setting-fold settings-wide" open>
        <summary><span class="setting-fold-title"><b>寿司郎认证</b><span>只在抢预约、远程取号、读取我的单据时需要。凭证会过期，也可能被手机端重新打开小程序后顶掉。</span></span></summary>
        <div class="setting-fold-body">
        <div class="fl ai jb mb16 fw g8"><div class="cd-t" style="margin-bottom:0">认证状态</div><div class="fl g8 fw"><button class="bt bt-r bt-s" onclick="openAuthWizard()">获取凭证向导</button><button class="bt bt-o bt-s" onclick="resetAuthOnly(true)">重置认证</button><button class="bt bt-w bt-s" onclick="testAuthProbe()">测试基础接口</button></div></div>
      ...
      <details class="cd setting-fold settings-wide">
        ...
        <div class="setting-fold-body">
        <div class="fl ai jb mb16 fw g8"><div class="cd-t" style="margin-bottom:0">本机诊断</div><div class="fl g8 fw"><button class="bt bt-w bt-s" onclick="lD()">刷新</button><button class="bt bt-w bt-s" onclick="copyDiag()">复制诊断</button><button class="bt bt-r bt-s" onclick="repairP()">修复代理</button></div></div>
      ...
      <div class="debug-strip settings-wide">
        <div><b>调试模式</b><span>普通用户默认关闭。开启后才显示原始凭证导入、移动端 UA、接口发现、云端服务地址等开发/排障工具。</span></div>
        <label class="switch"><input type="checkbox" id="debugModeToggle" onchange="setDebugMode(this.checked)"> 显示调试工具</label>
      </div>
      <details class="cd setting-fold debug-only settings-wide">
...
<script>
let cp='da',es={status:'idle'},hc=0,as=[],sd='',pr={},pf='',cE=null,stores=[],selStores=[],calErrs=[],arTimer=null,lastDiag=null,spCfg={},spState={status:'idle'},spAutoStart={},spQueueState={},qdSelected=[],qtSelected=[],qtTrendStores=[],qaStatus={},ah={},nfc=true,notifyChannels=[],cloudAuth={},debugMode=false,qpStore='',qwStore='';
...
const NAV_GROUPS=[
  ...
  {id:'settings',label:'设置',pages:[['se','设置'],['sm','预测准确度'],['lo','日志']]}
];
...
async function loadStatus(){try{const r=await(await fetch('/api/status')).json();el('ver').textContent='v'+r.version;hc=!!r.has_config;pf=r.platform||'';es=r.engine||{status:'idle'};spState=r.sampling||spState;ah=r.auth_health||{};nfc=r.notify_configured!==false;uE();uSamplingSummary();uD();uAuth();renderSettingsStatus();renderSettingsDataState();}catch(e){el('ver').textContent='offline';}}
function uAuth(){const pill=el('authPill'),banner=el('authBanner'),st=(ah&&ah.status)||'unknown',reason=(ah&&ah.reason)?String(ah.reason):'',needToday=hc&&st!=='stale'&&ah&&ah.routine_ready===false;if(pill){if(!hc){pill.className='authpill';pill.textContent='只读模式'}else if(st==='stale'){pill.className='authpill stale';pill.textContent='凭证可能过期'}else if(needToday){pill.className='authpill stale';pill.textContent='需今日认证'}else{pill.className='authpill ok';pill.textContent='今日凭证'}pill.classList.remove('hid')}if(banner){if(hc&&st==='stale'){banner.classList.remove('hid');banner.innerHTML='<b>凭证可能已过期</b>'+(reason?esc(reason)+'。':'')+'寿司郎凭证会过期，也可能被手机端登录顶掉。请先重置认证，再重新获取凭证。<button class="bt bt-r bt-s" onclick="event.stopPropagation();resetAuthAndStart()">重置并重新认证</button>'}else if(needToday){banner.classList.remove('hid');banner.innerHTML='<b>Routine 需要今天重新认证</b>'+esc((ah&&ah.routine_reason)||'为避免自动取号到点失败，请今天重新获取寿司郎凭证。')+'<button class="bt bt-r bt-s" onclick="event.stopPropagation();resetAuthAndStart()">今天重新认证</button>'}else{banner.classList.add('hid');banner.innerHTML=''}}}
function authPillClick(){if(hc&&(ah&&ah.status==='stale'||ah&&ah.routine_ready===false))resetAuthAndStart();else if(!hc)startAuth();else go('re')}
async function init(){initDebugMode();consumeCloudAuthResult();await loadStatus();await lP();checkUpdate();sse();const h=location.hash.slice(1);if(h&&PAGE_GROUP[h]&&h!=='da')go(h);else maybeShowIntro();}
...
function initDebugMode(){try{debugMode=localStorage.getItem('sushiro_debug_mode')==='1'}catch(e){debugMode=false}applyDebugMode()}
function setDebugMode(on){debugMode=!!on;try{localStorage.setItem('sushiro_debug_mode',debugMode?'1':'0')}catch(e){}applyDebugMode();toast(debugMode?'调试模式已开启':'调试模式已关闭')}
function applyDebugMode(){document.body.classList.toggle('debug-mode',!!debugMode);['debugModeToggle'].forEach(id=>{const x=el(id);if(x)x.checked=!!debugMode})}
...
function renderSettingsStatus(){
 ...
 const stale=hc&&ah&&ah.status==='stale',routineAuthNeed=hc&&!stale&&ah&&ah.routine_ready===false,authCls=!hc||stale?'bad':routineAuthNeed?'warn':'ok',authValue=!hc?'未认证':stale?'可能过期':routineAuthNeed?'需今日刷新':'今日可用',authCopy=!hc?'取号、预约和我的单据需要先获取寿司郎凭证。':stale?'凭证可能过期或被手机端登录顶掉，建议重置后重新认证。':routineAuthNeed?(ah.routine_reason||'Routine 自动取号要求当天重新认证。'):'取号、预约和我的单据可用；凭证仍可能后续过期。';
 ...
 box.innerHTML=settingsCard('寿司郎认证',authValue,authCopy,authCls,!hc||stale||routineAuthNeed?'<button class="bt bt-r bt-s" onclick="resetAuthAndStart()">重新认证</button>':'<button class="bt bt-w bt-s" onclick="go(\'re\')">看我的单据</button>')+settingsCard('GitHub 线上基准',cloudValue,cloudCopy,cloudCls,cloudConn?'<button class="bt bt-o bt-s" onclick="logoutCloudAuth()">退出</button>':'<button class="bt bt-w bt-s" onclick="startCloudLogin()">登录 GitHub</button>')+settingsCard('通知提醒',channels,notifyCopy,notifyCls,'<button class="bt bt-w bt-s" onclick="focusNotifySettings()">配置通知</button><button class="bt bt-w bt-s" onclick="tN(\'all\')">测试</button>')+settingsCard('预测数据',dataValue,dataCopy,dataCls,'<button class="bt bt-w bt-s" onclick="go(\'sm\')">配置</button>');
 if(acts){
  const a=[];if(!hc||stale||routineAuthNeed)a.push('<button class="bt bt-r" onclick="resetAuthAndStart()">重置并重新认证</button>');if(!nfc)a.push('<button class="bt bt-w" onclick="focusNotifySettings()">配置通知</button>');if(!cloudConn)a.push('<button class="bt bt-w" onclick="startCloudLogin()">登录 GitHub</button>');if(!(spState&&spState.running))a.push('<button class="bt bt-w" onclick="go(\'sm\')">提升预测准确度</button>');
...
async function refreshQueueView(){await loadQueueLive();loadQueueRecommend();await loadNetTicketRoutine();await loadNetTicketPlan();await loadQueueAlerts();await loadQueueAlertStatus();await loadQueueBaseline();await loadQueueTrends()}
...
async function loadNetTicketRoutine(){try{const d=await safeFetch('/api/queue/ticket/routine');ntRoutine=(d&&d.routine)||{};if(d&&d.plan)ntPlan=d.plan;fillNetTicketStores();if(el('nrStore')&&ntRoutine.store_id)el('nrStore').value=String(ntRoutine.store_id);if(el('nrMeal')&&ntRoutine.target_meal_time)el('nrMeal').value=netTimeDisp(ntRoutine.target_meal_time);if(el('nrTravel'))el('nrTravel').value=ntRoutine.travel_minutes||0;if(el('nrSafety'))el('nrSafety').value=(ntRoutine.safety_minutes==null?10:ntRoutine.safety_minutes);renderNetTicketRoutineStatus(ntRoutine)}catch(e){}}
function renderNetTicketRoutineStatus(r){
 const box=el('nrStatus');if(!box)return;r=r||{};
 if(!r.enabled){box.innerHTML='<span class="mu">未开启 Routine。开启后，本工具会每天按目标就餐时间倒推取号时间；样本不足时不会自动取号。</span>';return}
 const store=esc(r.store_name||storeDisplayName(r.store_id)||r.store_id||''),meal=r.target_meal_time?netTimeDisp(r.target_meal_time):'-',pickup=r.planned_pickup_time||'',range=r.recommend_pickup_range?(r.recommend_pickup_range.early+'-'+r.recommend_pickup_range.late):'',wait=r.wait_minutes_range?('预计等 '+r.wait_minutes_range.low+'-'+r.wait_minutes_range.high+' 分钟'):'等待样本不足',risk=r.risk==='high'?'风险偏高':r.risk==='medium'?'风险中等':r.risk==='low'?'风险较低':'风险待确认';
 let head='',detail='';
 switch(r.status){
  case'armed':head='已开启：今天预计 '+pickup+' 自动取号';detail=store+' · 目标 '+meal+' 吃 · '+wait+(range?' · 建议窗口 '+range:'')+' · '+risk;break;
  case'needs_auth':head='已开启：需要今天重新认证';detail=(r.last_error||'寿司郎凭证会过期或被手机端登录顶掉，请今天重新认证后再让 Routine 自动取号。')+(pickup?' 预计取号时间 '+pickup+'。':'');break;
  case'waiting_data':head='已开启：等待历史样本';detail=r.last_error||'这家店样本不足，暂不自动取号。去“预测准确度”开启本机采集后会自动补齐。';break;
  case'blocked':head='已开启：被手动计划暂停';detail=r.last_error||'已有手动自动取号计划，Routine 不会覆盖它。';break;
  case'missed':head='今天已错过推算取号时间';detail=r.last_error||'Routine 明天会重新规划。';break;
  case'done':head='今天已处理过自动取号';detail=r.last_error||'取号结果请看“我的单据”。';break;
  case'error':head='Routine 保存失败';detail=r.last_error||'未知错误';break;
  default:head='已开启：等待下一次规划';detail='目标 '+meal+' 吃，后台会按历史等待倒推取号时间。'
 }
 const authBtn=r.status==='needs_auth'?'<button class="bt bt-r bt-s" onclick="resetAuthAndStart()">今天重新认证</button>':'';
 box.innerHTML='<b>'+esc(head)+'</b><div class="mu mt8">'+esc(detail)+(r.basis?'<br>'+esc(r.basis):'')+'</div><div class="fl g8 fw mt8">'+authBtn+'<button class="bt bt-w bt-s" onclick="go(&quot;sm&quot;)">提升预测准确度</button><button class="bt bt-w bt-s" onclick="refreshNetTicketRoutineNow()">重新试算今天</button></div>'
}
...
async function refreshNetTicketRoutineNow(){if(!ntRoutine||!ntRoutine.enabled){await loadNetTicketRoutine();return}try{const d=await safeFetch('/api/queue/ticket/routine',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:true,store:ntRoutine.store_id,store_name:ntRoutine.store_name||storeDisplayName(ntRoutine.store_id),target_meal_time:ntRoutine.target_meal_time,travel_minutes:ntRoutine.travel_minutes||0,safety_minutes:ntRoutine.safety_minutes==null?10:ntRoutine.safety_minutes})});if(d.error){toast(d.error);return}ntRoutine=d.routine||{};if(d.plan)ntPlan=d.plan;fillNetTicketStores();renderNetTicketRoutineStatus(ntRoutine);renderNetTicketStatus(ntPlan);toast('已重新试算今天')}catch(e){toast('重新试算失败')}}
async function saveNetTicketRoutine(enabled){const sel=el('nrStore'),store=sel?sel.value:'',meal=el('nrMeal')?.value||'',travel=Math.max(0,parseInt(el('nrTravel')?.value||'0',10)||0),safety=Math.max(0,parseInt(el('nrSafety')?.value||'0',10)||0);if(enabled){if(!store){toast('请先选门店');return}if(!meal){toast('请填想几点吃');return}if(!await confirmDialog('启用每日到店 Routine？\\n每天会按目标就餐时间倒推取号时间，并在到点后自动远程取号。\\n取到号后请尽快到店；这不是只读功能。'))return}else if(ntRoutine&&ntRoutine.enabled){if(!await confirmDialog('关闭每日到店 Routine？\\n这只会停止未来自动规划/自动取号，不会取消已经拿到的排队号。'))return}const sn=(sel&&sel.selectedOptions[0])?sel.selectedOptions[0].textContent:'';try{const d=await safeFetch('/api/queue/ticket/routine',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:enabled,store:store,store_name:sn,target_meal_time:compactTime(meal),travel_minutes:travel,safety_minutes:safety})});if(d.error){toast(d.error);return}ntRoutine=d.routine||{};if(d.plan)ntPlan=d.plan;fillNetTicketStores();renderNetTicketRoutineStatus(ntRoutine);renderNetTicketStatus(ntPlan);toast(enabled?'已开启 Routine，会按历史等待倒推每天取号时间':'已关闭 Routine')}catch(e){toast('保存 Routine 失败')}}
...
async function loadSampling(){try{const d=await(await fetch('/api/sampling')).json();spCfg=d.config||{};spState=d.state||{};spAutoStart=d.autostart||{};spQueueState=d.queue_state||{};fillSamplingForm();renderSamplingStores();renderSamplingState();uSamplingSummary();renderDashboardSamplingCard()}catch(e){el('sampleState').innerHTML='<div class="ci bad">预测准确度状态加载失败</div>';renderDashboardSamplingCard()}}
...
async function saveSampling(quiet){spCfg=samplingPayload();try{const d=await(await fetch('/api/sampling',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(spCfg)})).json();if(d.error){if(!quiet)toast(d.error);return false}spCfg=d.config||spCfg;spState=d.state||spState;renderSamplingStores();renderSamplingState();uSamplingSummary();renderDashboardSamplingCard();if(!quiet)toast(spState.running?'预测配置已保存，后台已按新配置重启':'预测配置已保存');return true}catch(e){if(!quiet)toast('保存失败');return false}}
async function startSampling(){if(el('spEnabled'))el('spEnabled').checked=true;if(!await saveSampling(true))return;try{const d=await(await fetch('/api/sampling/start',{method:'POST'})).json();if(d.error){toast(d.error);return}spState=d.state||spState;await loadSampling();uSamplingSummary();toast('已开启本机持续采集')}catch(e){toast('启动失败')}}
...
async function startCloudLogin(){const base=(el('cloudUrl')?.value||'').trim();if(base&&base!==(cloudAuth.base_url||'')){if(!await saveCloudAuth())return}if(!(cloudAuth.configured||base)){setDebugMode(true);toast('还没有配置云端服务地址；自建服务请在调试模式里填写地址。');return}location.href='/api/cloud/auth/start'}
