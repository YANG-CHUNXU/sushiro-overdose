...
// authHealth 跟踪"本机捕获的认证令牌是否还有效"。背景：工具复用的是手机/PC 微信抓包
// 得到的同一套令牌，寿司郎一个账号同一时间只认一个活跃会话——在手机上用过小程序后，
// 电脑这边的令牌就会被顶失效。这里用被动方式检测（只看用户已触发的认证请求结果，
// 不做任何后台轮询，避免再把手机会话顶掉），并在 ok→stale 跃迁时提醒一次。
// 详见 specs/006-auth-staleness-reminder。

const (
	authHealthUnknown = "unknown" // 尚无判断（含从未捕获）
	authHealthOK      = "ok"      // 最近一次认证请求成功
	authHealthStale   = "stale"   // 最近一次认证请求被官方判为认证失败
)
...
// AuthHealthJSON 是 /api/status 暴露的认证健康。
...
// markAuthHealthy：一次认证请求成功，或刚重新捕获/导入认证后调用。清除 stale 与通知去重。
...
// markAuthStale：官方判定认证失败时调用。仅在 ok/unknown→stale 跃迁时推一次通知。
func markAuthStale(reason string) {
	...
	if shouldNotify {
		sendNotification("寿司郎 - 认证可能已过期",
			"在手机上用过寿司郎小程序后，电脑这边的认证就会失效（同一账号只认一个会话）。请在工具里重新获取认证。")
	}
}

// noteAuthResult：被动检测入口。把一次"需要认证的请求"的结果喂进来——
// err 为认证失败 → stale；err 为 nil（成功）→ healthy；其它错误不改变认证健康。
func noteAuthResult(err error) {
	...
	if isAuthError(err) {
		markAuthStale("官方接口返回认证失败（401/403）")
	}
}
